					...
					<!--tab切换规格-->
						...
						<div v-if='num2==0'>
							<div class='gg-border' >
								...
								<span class='two' v-for='(item,index) in moreAttrs' @click='clickCheckbox(item,index,$event)' style='cursor:pointer;'>
									<span>
										<input type='checkbox' v-model="item.checked"  @change='selectedAttr($event,item,index)'/>{{item.name}}
									</span>
							...
							<div class='gg-border1'  v-for='(i,index) in addAttrs' ><!--addAttrs-->
								<span class='first'>
									{{i.name}}
								</span>
...
import Uediter from '@/components/ue.vue'
	export default {
		...
		editspecCombinations(){
			...
			getmoreAttrs(){
				this.axios.get('skuspec/list?uid='+this.uid).then(res=>{
						if(res.data.status==200){
							...
							// this.moreAttrs=JSON.parse(JSON.stringify(res.data.rows))
			...
			//属性管理
			gotoattrsetting(){
				this.$router.push('/managesettingdetails')
			},
			//详情tabs切换
			addborder(index){
				this.num2 = index
