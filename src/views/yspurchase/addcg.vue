...
<!--商品属性弹框-->
	<Modal v-model="attrModel" width="500px" :closable="false" >
		<p slot="header" style="height:30px;line-height:30px">
			...
			<Table  border  height='300' highlight-row  :columns="sAttrskey" :data="sAttrsData" ></Table>
		<div slot="footer">
			<div >
				<div>
	...
	export default {
			...
			getDefaultAnddetails(id){
				...
				this.axios.get('/po/get/'+id+'?uid='+this.uid).then((res)=>{
					...
					if(rows.status==88){
						...
						if(rows.deliveryDate==null||rows.deliveryDate==undefined){
							this.deliveryDate=''
						}else{
							this.deliveryDate=this.getLocalTime(rows.deliveryDate)
						}

						if(rows.etdDate==null||rows.etdDate==undefined){
							this.etdDate=''
						}else{
						...
						this.cgxqInfo.createTime=this.getLocalTime(this.cgxqInfo.createTime)
						this.cgxqInfo.deliveryDate=this.getLocalTime(this.cgxqInfo.deliveryDate)
					...
					if(res.data.status==200){
						...
						this.detailVoList.forEach((x,index)=>{
								...
								let arr={
									...
									skuTypeId:this.detailVoList[index].skuTypeId,
									skuTypeDesc:this.detailVoList[index].skuTypeDesc,
									zoneNo:"",
									zoneId:"",
									locationNo:"",
									locationId:""
        ...
        mounted:function() {
				...
				if(this.$route.params.item){
					...
					rows.soDetailList.forEach((x,index)=>{
						rows.soDetailList[index].costPrice=rows.soDetailList[index].salesPrice
					})
