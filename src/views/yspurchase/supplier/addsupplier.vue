<template>
      ...
      <div class="heade">
           <!--  <div class="top"> -->
                <!-- <Col span="6"> -->
                    <div class="left-top">
                      <p class="wldw">
                            <span style="width:42px;font-weight:900">{{$t('public.screen')}}：</span>
                             <Select style="width:100px"  v-model='valueT'  @on-change="orderselected">
                    ...
                    <div class="right-top">
                        <span style="margin-left:10px;font-size: 19px;font-weight: 600;position:absolute:left:360px">{{unitsText}}</span>
                	</div>
                	<div style="position: absolute;right: 260px;height: 100%;line-height: 60px;">
                        <div style='position:relative;display:flex;margin-right:20px' v-if="addText">
                            
			                <div style='display:inline-block;'>
	                            <Button type="primary" shape="circle" style="margin-right:5px;border:none;background:#3b77e3;color:#fff"  @click="saveBaseInfo('formValidate')" v-if="addText"><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>{{$t('public.SaveBtn')}}</Button><!--保存-->
	                            <Button type="primary" shape="circle" style="background:#999999;color:#fff;margin-right:10px;"   @click="cancel" v-if="addText" ><Icon type="close" style="margin-right:5px;"></Icon>{{$t('public.cancel')}}</Button>
	                             <span @click="cancelHome" v-if="addText" style="cursor:pointer;"><Icon type="close" class="cha" ></Icon></span>
                            </div>
                        </div>
                        <div v-if="!addText" style='margin-right:20px'>
                        	<div style='position:relative;display:flex'>
                        		 
				                <div>
                            <Button type="primary" shape="circle" style="background:#3b77e3;margin-right:5px;color:#fff;border:none"  @click="glsku" v-if="xiugaibtn1"><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>
                            {{$t('purchase.supplier.Asku')}}<!--关联商品-->
                        </Button>
                            <!-- <Button type="primary" shape="circle" style="background:#3b77e3;margin-right:5px;color:#fff;border:none"  @click="loginSetting" v-if="xiugaibtn1"><Icon type="person" style=";margin-right:5px;color:#fff"></Icon>
                            {{$t('purchase.supplier.Amanage')}}</Button>账号管理-->
                             <Button type="primary" shape="circle" style="background:#40ca98;margin-right:5px;color:#fff;border:none"  @click="changeUnitInfo1" v-if="xiugaibtn1"><i class="el-icon-edit" style=";margin-right:5px;"></i>
                             {{$t('public.EditBtn')}}</Button><!--修改-->

                               <Button type="primary" shape="circle" style="background:#3b77e3;margin-right:5px;color:#fff;border:none"  @click="changeUnitInfo2('formValidate1')" v-if="xiugaibtn2"><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>{{$t('public.SaveBtn')}}</Button>
                         
                             <Button type="primary" shape="circle" style="background:#999999;color:#fff;margin-right:10px;"   @click="cancel" v-if="addText" ><Icon type="close" style="margin-right:5px;"></Icon>{{$t('public.cancel')}}</Button>
                             <span @click="cancelHome" v-if="addText" style="cursor:pointer;margin-left: 10px;"><Icon type="close" class="cha" ></Icon></span>
                            </div>

						<div style=";display:flex">
                            <div v-if="newBusiness" class="cursor">
				                ...
				                <DropdownMenu slot="list">
				              		<span @click="storeImport"><DropdownItem >{{$t('public.import')}}</DropdownItem></span><!--导入-->
				              		<span @click="Invitation"><DropdownItem >邀请</DropdownItem></span>
				                    <span @click="start" v-if='isstart'><DropdownItem >{{$t('public.start')}}</DropdownItem></span>
				                    <span @click="stopUnits" v-if='isstop'><DropdownItem >{{$t('public.stop')}}</DropdownItem></span>
				                    <span @click="delunit" v-if='issdel'><DropdownItem >{{$t('public.del')}}</DropdownItem></span>
			...
			<!--左边往来单位列表-->
				...
				<div class="left-content">
					<div class='locationSearch'>
						<Input style='padding:10px 20px;' v-model='searchKeyword' :placeholder="$t('public.orderorname')" @on-enter='enterSearch'>
					...
					<Scroll :on-reach-bottom="handleReachEdge" :height="height" v-else>
						...
						<div class="con-list" v-for="(item,index) in Allunits" :key='index' @click="updataUnit(item.id,index)" :class="{bg:numLeftList==index}">
							...
							<p class="list-title">
								<span>{{item.code}}</span>
								<span class='file' >
									<Tooltip :content="$t('public.isfile')" placement="right-start">
				...
				<div class="add-right">
					<!-- <div class="right-content"  v-if="addText"> -->
						<div  style="width:100%;margin-bottom:100px;padding:20px 10px 0 10px" v-if="addText">
								<Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
									<el-collapse v-model="activeName" accordion>
										<el-collapse-item  title="供应商信息" name="1">
											<Row>
				                        		<Col span="8">
					                        		<FormItem :label="$t('purchase.supplier.code')" prop="code"><!--编码-->
							                        	<Input v-model="formValidate.code" :placeholder="$t('manage.warehouse.codeplaceholder')"></Input>
							                   		 </FormItem>
						                   		</Col>
						                   		<Col span="8">
													<FormItem :label="$t('purchase.supplier.name')" prop="name">
							                        	<Input v-model="formValidate.name" :placeholder="$t('purchase.supplier.name')"></Input>
							                   		 </FormItem>
						                   		</Col>
						                   		<Col span="8">
												 	<FormItem :label="$t('purchase.supplier.level')" prop="credits">
						                        	<Select  v-model='formValidate.level' :placeholder="$t('public.all')" @on-change="changekhlevel" >
								                        <Option v-for="(item,index) in khlevel" :value="item.value" :key="index">{{ item.label }}
								                        </Option>
								                    </Select>
						                   		 	</FormItem>
						                   		</Col>
						                   		<Col span="8">
							                   		<FormItem :label="$t('manage.supplier.payType')" prop="payType">
						                   		...
						                   		<Col span="8">
							                   		<FormItem :label="$t('purchase.supplier.zq')" prop="paymentDay"><!--账期-->
							                        	<Input v-model="formValidate.paymentDay" number :placeholder="$t('purchase.supplier.zq')"></Input>
							                   		 </FormItem>
						                   		</Col>
						                   		<Col span="24">
							                   		<FormItem :label="$t('purchase.supplier.Raddress')"><!--地址-->
															...
															<Input type="text" v-model="address" :placeholder="$t('purchase.supplier.Raddress')"></Input>
														</div>
													</FormItem>
												</Col>
												<Col span="8">
													<FormItem :label="$t('purchase.supplier.contract')" prop="contract">
							                        		<Input v-model="formValidate.contract" :placeholder="$t('purchase.supplier.contract')"></Input><!--联系人-->
							                   		</FormItem>
						                   		</Col>
						                   		<Col span="8">
						                   		 <FormItem :label="$t('purchase.supplier.contractPhone')" prop="contractPhone">
						                        	<Input v-model="formValidate.contractPhone" :placeholder="$t('purchase.supplier.contractPhone')"></Input><!--联系人手机-->
						                   		 </FormItem>
						                   		</Col>
											</Row>
										</el-collapse-item>
									...
									<el-collapse>
										<el-collapse-item title="联系人信息" name="2">
											<Row>
						                   		 ...
						                   		 <Col span="8">
						                   		 <FormItem :label="$t('purchase.supplier.contractTel')" prop="contractTel">
						                        	<Input v-model="formValidate.contractTel" :placeholder="$t('purchase.supplier.contractTel')"></Input>
						                   		 </FormItem>
						                   		</Col>
						                   		 <Col span="8">
						                   		 <FormItem :label="$t('purchase.supplier.contractPosition')" prop="contractFax"><!--职位"-->
						                        	<Input v-model="formValidate.contractPosition" :placeholder="$t('purchase.supplier.contractPosition')"></Input>
						                   		 </FormItem>
						                   		</Col>
						                   		 <Col span="8">
						                   		 <FormItem :label="$t('purchase.supplier.contractEmail')" prop="contractEmail">
						                        	<Input v-model="formValidate.contractEmail" :placeholder="$t('purchase.supplier.contractEmail')"></Input>
						                   		 </FormItem>
						                   		</Col>
						                   		 <Col span="8">
						                         <FormItem :label="$t('purchase.supplier.Raddress')" prop="contractAddress">
						                        	<Input v-model="formValidate.contractAddress" :placeholder="$t('purchase.supplier.Raddress')"></Input>
						                   		 </FormItem>

												</Col>
											</Row>
						                    <FormItem :label="$t('public.remark')" >
				...
				<!--供应商修改-->
					<div  style="width:100%;height:height:100%;padding:20px 10px 0 10px;margin-bottom:370px" v-if="xiugai">
				...
				<!--详情明细订单页面-->
                    <div class="right-content rigth-details" ref="details"  v-if='isdetails'>
                        	<div class="detail-top" style="padding: 0px 10px 0px 0px;">
	                        		<Form  :label-width="80">
	                        		...
	                        		<Row>
		                        		<Col span="8">
				                   		...
				                   		<Col span="8">
											<FormItem :label="$t('purchase.supplier.name')" prop="name">
					                        	<Input disabled v-model="formValidate.name" :placeholder="$t('purchase.supplier.name')"></Input>
					                   		 </FormItem>
				                   		</Col>
				                   		<Col span="8">
										 	<FormItem :label="$t('purchase.supplier.level')" prop="credits">
				                        	<Select disabled v-model='formValidate.level' :placeholder="$t('public.all')">
						                        <Option v-for="(item,index) in khlevel" :value="item.value" :key="index">{{ item.label }}
						                        </Option>
						                    </Select>
				                   		 	</FormItem>
				                   		...
				                   		<Col span="8">
					                   		<FormItem :label="$t('manage.supplier.payType')" prop="payType">
					                        	<Select v-model="formValidate.payType" disabled multiple>
				                   		...
				                   		<Col span="8">
					                   		<FormItem :label="$t('purchase.supplier.zq')" prop="paymentDay"><!--账期-->
					                        	<Input disabled v-model="formValidate.paymentDay" number :placeholder="$t('purchase.supplier.zq')"></Input>
					                   		 </FormItem>
				                   		</Col>
				                   		<Col span="16">
					                   		<FormItem :label="$t('purchase.supplier.Raddress')"><!--地址-->
													...
													<Input disabled type="text" v-model="address" :placeholder="$t('purchase.supplier.Raddress')"></Input>
												</div>
											</FormItem>
										</Col>
									</Row>
									<Row>
										<Col span="8">
											<FormItem :label="$t('purchase.supplier.contract')" prop="contract">
					                        		<Input disabled v-model="formValidate.contract" :placeholder="$t('purchase.supplier.contract')"></Input><!--联系人-->
					                   		</FormItem>
				                   		</Col>
				                   		<Col span="8">
				                   		 <FormItem :label="$t('purchase.supplier.contractPhone')" prop="contractPhone">
				                        	<Input disabled v-model="formValidate.contractPhone" :placeholder="$t('purchase.supplier.contractPhone')"></Input><!--联系人手机-->
				                   		 </FormItem>
				                   		</Col>
				                   		 <Col span="8">
				                   		 <FormItem :label="$t('purchase.supplier.contractTel')" prop="contractTel">
				                        	<Input disabled v-model="formValidate.contractTel" :placeholder="$t('purchase.supplier.contractTel')"></Input>
				                   		 </FormItem>
				                   		</Col>
				                   		 <Col span="8">
				                   		 <FormItem :label="$t('purchase.supplier.contractPosition')" prop="contractFax"><!--职位"-->
				                        	<Input disabled v-model="formValidate.contractPosition" :placeholder="$t('purchase.supplier.contractPosition')"></Input>
				                   		 </FormItem>
				                   		</Col>
				                   		 <Col span="8">
				                   		 <FormItem :label="$t('purchase.supplier.contractEmail')" prop="contractEmail">
				                        	<Input disabled v-model="formValidate.contractEmail" :placeholder="$t('purchase.supplier.contractEmail')"></Input>
				                   		 </FormItem>
				                   		</Col>
				                   		 <Col span="8">
				                         <FormItem :label="$t('purchase.supplier.Raddress')" prop="contractAddress">
				                        	<Input disabled v-model="formValidate.contractAddress" :placeholder="$t('purchase.supplier.Raddress')"></Input>
				                   		 </FormItem>

										</Col>
									</Row>
				                    <FormItem :label="$t('public.remark')" >
										<Input disabled v-model="remark" style="width:100%" type="textarea" :placeholder="$t('public.remark') " class="getaddress" :min-row="6"></Input>
			                   		 </FormItem>
                        		</Form>
                            	<div class="detail-title-mark" style='margin-top:20px'>
	                             ...
	                             <!--关联商品-->
	                             <div v-if='num4==0' style='margin:10px 10px 120px;width:99%'>
		                             	<Table  border height="300":columns="goodskey1" :data="selectedgoodsrow1" >
	              						</Table>
		                                <div style='margin-top:10px;margin-bottom:90px'>
		              						<Page size="small" :total="goodskey1total" show-total  :pageSize='goodskey1pageSize'  @on-change="goodskey1getData" @on-page-size-change="goodskey1changePageSize">
		              						</Page>
		                                </div>
	                             </div>
	                              <!--附件-->
                            <div v-if='num4==1' style='margin:10px 0 120px 0'>
                            	<div style='width:98%;margin-top:20px'>
	                            	<div>
								        ...
								        <div v-if='upData1==null'>
								        	<span>{{$t('public.nofile')}}</span><!--暂无附件-->
								        </div>
       ...
       <!--第一步-->
        <div style='width:100%;margin:20px auto;text-align:center;' v-if='oneStep'>
           <div style='width:100%;margin:20px auto;text-align:center;font-size:14px;font-weight:600;border-top:1px solid #e4e4e4'>
                <span style='display:block;margin: 20px auto 30px 0;'>{{$t('manage.warehouse.fileImport')}}</span>
           </div>
            <div style='width:100%;margin:20px auto;text-align:center;position:relative'>
                <div style='margin-left:-105px'>
                <span>{{$t('manage.warehouse.filechoose')}}：</span>
                 <Input  disabled style='width:200px' v-model='url'></Input>
                  <div style='position:absolute;top: 0px;right: 137px;height: 35px;width: 110px;overflow: hidden;'>
                     ...
                     :show-file-list='false'
                      ...
                      <Button size="small" type="primary" style='background:#169BD5'>{{$t('manage.warehouse.upload')}}</Button>
            ...
            <div style='margin-top:40px;display: flex;justify-content:space-between;;margin-bottom:-40px;font-size:14px'>
            	<Button  type="ghost" style='margin-left:20px;border:none;background:#169BD5;color:#fff'>
                     ...
                     <a :href="supplierup" style='color:#fff' @click.stop='stopsku($event)'>{{$t('manage.warehouse.fileD')}}</a><!--下载模板文件-->
                </Button>
                <span style="cursor:pointer" @click="cancelku">{{$t('public.cancel')}}</span>
            </div>
        </div>
        <!--第二步-->
    	...
    	<!--关联商品的弹框-->
		<Modal v-model="glskuisshow" width="875px">
			<p slot="header" style="height:30px;line-height:30px;">
				<span>{{$t('purchase.supplier.Asku')}}<!--关联商品--></span>
			</p>
			<div style="position:relative;text-align:center;width:100%;height:35px;line-height:35px;display:flex;">
				<Input v-model='skuKeyword' class="modelInput" @on-enter='serarsku' :placeholder="$t('purchase.supplier.name')" style="padding:2px 0"></Input>
          ...
          <!-- <Icon type="plus" style='margin-right:5px'></Icon>
					...
					<div>
						<Button shape="circle" type="ghost" style="display:inline-block;background:#ACACAC;border:none;color:#fff" @click='glskucancel'><Icon type="close" style="margin-right:5px;" ></Icon>{{$t('public.cancel')}}</Button>
						<Button shape="circle" type="ghost" style="display:inline-block;background:#3b77e3;color:#fff" @click="sureglsku"><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>{{$t('public.sure')}}</Button>
					</div>
				</div>
			</div>
		</Modal>
    ...
    <Modal v-model="psisshow" width="360px">
	    <p slot="header" style="height:30px;line-height:30px;text-align:center">
	     ...
	     <Form :label-width="100">
	        ...
	        <FormItem :label="$t('purchase.supplier.name')" >
	           <Input v-model='supplierName' ></Input>
	        </FormItem>
	        <FormItem :label="$t('purchase.supplier.contractPhone')" >
	    ...
	    <div slot="footer">
	      <div class='footer-mark'>
	        ...
	        <span ><a style='color:#3B77E3;font-size:14px' @click='sureSearchLocation'>{{$t('public.sure')}}</a></span><!--确认-->
	      </div>
	    </div>
  	</Modal>
  	<!--账号管理-->
	<Modal v-model="loginModel" width="400px">
      <p slot="header" style="height:30px;line-height:30px;text-align:center">
        <span>{{$t('purchase.supplier.Amanage')}}</span><!--账号管理-->
      </p>
       <Form ref="formValidatelogin" :model="formValidatelogin" :rules="ruleValidatelogin" :label-width="100">
        <FormItem :label="$t('purchase.supplier.accountName')" prop="loginname"><!--用户名-->
           <Input style='' disabled v-model='formValidatelogin.loginname' ></Input>
        </FormItem>

        <FormItem :label="$t('purchase.supplier.contractPhone')" ><!--手机号-->
           <Input style='' disabled v-model='user.mobile'></Input>
        </FormItem>
        <FormItem :label="$t('purchase.supplier.Vcode')" prop="loginphone"><!--验证码-->
          <div style='display:flex'>
           <Input style='width:100px;margin-right:20px' :placeholder="$t('purchase.supplier.VcodePlaceholder')" v-model='formValidatelogin.loginphone'></Input>
           <Button  type="primary" style="background:#3b77e3;border:none:color:#fff" v-if='yzm' @click='sendYzm'>{{$t('purchase.supplier.getVcode')}}</Button><!--获取验证码-->
           <Button  type="primary" style="background:#f5f5f5;border:none:color:#fff" v-if='yzm1' disabled>{{yzmnumber}}{{$t('purchase.supplier.getVcode1')}}</Button><!--秒后重试-->
           </div>
        </FormItem>
        <FormItem :label="$t('purchase.supplier.Rpassword')" prop="loginNewPass"><!--新密码-->
          <Input :placeholder="$t('purchase.supplier.Rpassword')" v-model='formValidatelogin.loginNewPass' type='password'></Input>
        </FormItem>
      </Form>
      <div slot="footer">
        <div class='footer-mark'>
          <span><a style='color:#999;font-size:14px' @click='cancelPs'>{{$t('public.cancel')}}</a></span>
          <span style='font-size:20px;width:1px;height:40px;background:#e4e4e4'></span>
          <span ><a style='color:#3B77E3;font-size:14px' @click='savePs("formValidatelogin")'>{{$t('purchase.supplier.pssReset')}}</a></span><!--密码重置-->
        </div>
      </div>
    </Modal>
	...
	export default {
		...
		data() {
			return {
				...
				skuKeyword:'',
				// title5:['可选商品','已选商品'],
				title5:[this.$t('purchase.supplier.kxsku'),this.$t('purchase.supplier.yxsku')],
				kxskudata:[],//可选商品数组
				yxskudata:[],//已选商品数组
				kxskukey:[
					{
				...
				// titlessku:['关联商品',"附件列表"],
				titlessku:[this.$t('purchase.supplier.Asku'),this.$t('purchase.supplier.Alist')],
				num4:0,
				chaIndex:-1,//附件index
				chaisshow:false,
                ...
                error:'',//导入失败的提示
                //供应商导入地址
				actionUrl1:LoadUrl.httpPrefix.Url+'partner/import?uid='+this.$store.state.common.token,
                status:'',//上传状态
                type:1,//选择文件类型
                oneStep:true,//第一步显示
                twoStep:false,//第2步显示
				...
				ruleValidate:{
					loginName:[ { required: true, message: '登陆账号不能为空', trigger: 'blur' }],
          			pass:[{ required: true, message:this.$t('public.pssEmpty'), trigger: 'blur' }, { pattern: /^[0-9a-zA-Z]+$/, message: '密码应欸数字或字母' },],
					name:[
				...
				numLeftList:-1,
				addNewUnits:"",
				unitsText:this.$t('purchase.supplier.addSupplier'),//"新增供应商",//新增供应商
				addText:true,
		...
		methods: {
			...
			addUnit(){
				...
				this.unitsText =this.$t('purchase.supplier.addSupplier'),// "新增供应商"
			...
			Invitation(){
				if(this.id==''){
					...
					this.axios.get('partner/invite/?mobile='+this.formValidate.contractPhone+'&uid='+this.uid).then(res=>{
						if(res.data.status==200){
							this.$notify({
						        title: '',
						        message: '邀请成功',
            ...
            //供应商等级
            getgyslevel(){
            	this.axios.get('partnerl/query?length=99&uid='+this.uid).then(res=>{
            		if(res.data.status==200){
           ...
           //点击停用
            ...
            handleReachEdge(){
				this.current++//默认10条
    	 		this.axios.get('/partner/query?cl='+this.valueT+'&offset='+this.current+'&uid='+this.uid).then((res)=>{
            ...
            delUp(item,index){
				...
				let s1 = strvalue.join(',')
				this.attrid = s1//id链接后字符串
            	this.axios.post('/file/action/delete?key='+item.key+'&uid='+this.uid).then(res=>{
            		if(res.data.status==200){
		      ...
		      goodskey1getData(current){
		          this.axios.get('partner/sku/'+this.id+'?offset='+current+'&uid='+this.uid).then((res)=>{
						...
						this.yxskudata.forEach((x,index)=>{
							...
							x.partnerId=this.yxskudata[index].skuVo.partnerId
		...
		mounted() {
			this.supplierup=LoadUrl.actionLoad.supplier//供应商导入模版
			this.getUnitsList(this.valueT)
