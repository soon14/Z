...
<template>
	...
	<div class="add-box">
		...
		<div class="head">
				<div style='position:relative;width:100%;border-bottom: 1px solid #E6E6E6;'>
					...
					<div style="position: absolute;right: 240px;height: 100%;line-height: 60px;">
					<div v-if="addText" >
						<div style='width: 95px;text-align: right;;margin-left:20px;display:inline-block;position:relative;top:14px;height:35px;line-height:35px;overflow:hidden;z-index-1' v-if='upBtn'>
			                  <el-upload
			                    class="upload-demo"
			                    :action="actionUrl"
			                    :on-success='success'
						...
						<Button shape="circle"  @click="showDetail"  style="background:#999999;color:#fff;margin-right:10px;" ><Icon type="close" style="margin-right:5px;"></Icon>{{$t('public.cancel')}}</Button>
					...
					<div v-if="!addText" class="changebutton" >
						...
						<div style='width: 80px;margin-left:20px;display:inline-block;position:relative;top:14px;height:35px;line-height:35px;overflow:hidden;z-index-1' v-if="editshow">
			                  <el-upload
			                    class="upload-demo"
			                    :action="actionUrl"
			                    :on-success='success'
			                    ...
			                    :show-file-list='false'
			                    >
			                    <Button shape="circle" type="primary" style="background:#3b77e3;border:none:color:#fff"> <span style='color:#fff'>上传附件</span></Button><!--上传附件-->
			                  </el-upload>
			                </div>
						...
						<!-- <Button type="ghost"  shape="circle"  @click="shouhuo" v-if='shouhuoshow' style="background:#3b77e3;margin-right:5px;color:#fff;border:none">
							<Icon type="checkmark-round" style="position:relative;right:5px"></Icon>
						收货
						</Button> -->
						...
						<!--更多-->
			                ...
			                <DropdownMenu slot="list" v-if="isdelisshow">
	...
	<div class="left-content">
		<div class='locationSearch'>
            ...
            <span slot="append"  @click='search' style='cursor:pointer;'>{{$t('public.search')}}</span>
             ...
             <!-- <a style='font-size:14px;font-weight:600;margin-left:20px;border-bottom:1px solid #3b77e3;height:35px;line-hegiht:35px;line-height: 35px;margin-top: -2px;' @click='searchLocation'>{{$t('public.PreciseSearch')}}</a> -->
		</div>
		...
		<Scroll :on-reach-bottom="handleReachEdge" :height="height" v-else >
			<div class="con-list" v-for="(item,index) in Allpo" :key='index' @click="getcginfo(item.id,index,item)" :class="{bg:numIndex==index}">
				...
				<div class="lanrenLeft" :class="{bgd5:item.status==1,bgd6:item.status==99,bgd7:item.status==88,bgd8:item.status==15,bgd18:item.statusDesc=='签收中'}">
	...
	<div class="right">
					...
					<div class="add-right">
						<div class="right-content" ref="divsave">
							...
							<div v-if="addText" style='margin-bottom:260px'>
								...
								<Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="90">
									...
									<Col span="8" style='margin-top: 80px;'>
										<FormItem :label="$t('purchase.purchaseOrder.supplier')" prop="supplierName">
											<!-- <span  @click="alertchModal">
												<Input v-model="formValidate.supplierName" :placeholder="$t('purchase.purchaseOrder.supplier')"></Input>
				                    ...
				                    <Col span="8" style='margin-top: 80px;'>
				                        <FormItem :label="$t('purchase.purchaseOrder.type')">
											<Select @on-change="getcgType" >
											<Option v-for="(item,index) in datacgList" :value='item.value' :key='item.value'>{{item.label}}</Option>
											</Select>
				                        </FormItem>
				                    </Col>
				                    <!-- <Col span="8" style='margin-top: 80px;'>
										<FormItem :label="$t('purchase.purchaseOrder.type1')">
			                       ...
			                       <FormItem :label="$t('purchase.purchaseOrder.date1')">
									...
									<Col span="8">
										<FormItem :label="$t('purchase.purchaseOrder.ck')" prop="warehouseName">
											<!-- <span  @click="warehouseModal">
												<Input v-model="formValidate.warehouseName" :placeholder="$t('purchase.purchaseOrder.ck')"></Input>
												<Icon type="ios-search" size="16" style='position:absolute;top:8px;right:8px'></Icon>
											</span> -->
				                    ...
				                    <Col span="8">
										<FormItem :label="$t('purchase.purchaseOrder.person')" prop="deliverName">
											<Input v-model="formValidate.deliverName" :placeholder="$t('purchase.purchaseOrder.person')"></Input>
					                    </FormItem>
				                    </Col>
				                    <Col span="8">
					                    <FormItem :label="$t('purchase.purchaseOrder.mobile')" prop="deliverMobile">
							...
							<div style='margin: 10px 0px 10px 20px;'>
								<div style='margin:10px 0'><span>{{$t('public.remark')}}：</span></div>
					...
					<div class="right-content" ref="shouh">
							...
							<div v-if="shouhshow" style='margin-bottom:260px'>
								<Row>
									...
									<Col span="12" style='margin-top: 80px;'>
										<span class='addInfo'>{{$t('purchase.purchaseOrder.purchaseInfo')}}</span>
										<FormItem :label="$t('purchase.purchaseOrder.supplier')" >
											<Input v-model="cgxqInfo.supplierName" disabled :placeholder="$t('purchase.purchaseOrder.supplier')"></Input>
											<Icon type="ios-search" size="16" style='position:absolute;top:8px;right:8px'></Icon>
				                        </FormItem>
				                        <FormItem :label="$t('purchase.purchaseOrder.ck')" >
											
												<Input v-model="cgxqInfo.deliverWarehouseName" disabled :placeholder="$t('purchase.purchaseOrder.ck')"></Input>
											
				                        </FormItem>
				                       
			                        <FormItem :label="$t('purchase.purchaseOrder.date')">
										<DatePicker type="date" :value='cgxqInfo.poDate' :placeholder="$t('purchase.purchaseOrder.date')" style="width:100%" @on-change="shdatepoDate"></DatePicker>
			                        </FormItem>
								</Col>

								<Col span="12" style='margin-top: 80px;'>

									<span class='addInfo'>{{$t('purchase.purchaseOrder.personInfo')}}</span>
									<FormItem :label="$t('purchase.purchaseOrder.person')" >
										<Input v-model="cgxqInfo.deliverName" :placeholder="$t('purchase.purchaseOrder.person')"></Input>

				                    </FormItem>
				                    <FormItem :label="$t('purchase.purchaseOrder.mobile')" >
										<Input v-model="cgxqInfo.deliverMobile" :placeholder="$t('purchase.purchaseOrder.mobile')"></Input>
				                    </FormItem>
				                     <FormItem :label="$t('purchase.purchaseOrder.address')" >
										<Input v-model="cgxqInfo.deliverAddress" :placeholder="$t('purchase.purchaseOrder.address')"></Input>
							...
							<!--选择商品表格-->
							<div class="table">
								<Table size="small" :highlight-row="true" border :columns="cunhuo2" :data="cunhuodata" @on-row-dblclick="clickchrow"></Table>
							</div>
					...
					<div class="right-content" ref="edit">
						...
						<div v-if="xiugai" style='margin-bottom:260px'>
									...
									<Col span="8">
										<FormItem :label="$t('purchase.purchaseOrder.ck')" prop="warehouseName">
											<div style='position:relative;'>
												...
												<div style="position:absolute;z-index:999;width:100%">
													<Table v-if="warehouseshow1" border height='300' highlight-row  :columns="ckkey" :data="allwarehouse" 
														@on-row-click="ckrow1" @on-row-dblclick="dbckrow">
														</Table>
												</div>
											</div>
				                        </FormItem>
							...
							<!--选择商品表格-->
							<div class="table">
								<!-- <Table size="small" :highlight-row="true" border :columns="cunhuoE" :data="detailVoList" @on-row-dblclick="clickchrow"></Table> -->
								<Table size="small" :highlight-row="true" border :columns="cunhuoE" :data="cgListInfo" @on-row-dblclick="clickchrow"></Table>
							</div>
							<div style='margin: 10px 0px 10px 20px;'>
								<div style='margin:10px 0'><span>{{$t('public.remark')}}：</span></div>
								<Input v-model="remark" style="width:100%" type="textarea" :placeholder="$t('public.remark')" class="getaddress" :min-row="6"></Input>
						...
						<div style='position:relative'>
							...
							<Form  :label-width="90">
									<Row>
					                   ...
					                   <!--  <Col span="8" style='margin-top: 20px;'>
											<FormItem :label="$t('purchase.purchaseOrder.type1')">
												<Select v-model="modeldztype"  disabled>
													<Option v-for="item in dztype" :value="item.value" :key="item.value">{{item.label}}</Option>
												</Select>
				                        	</FormItem>
				                         </Col> -->
								...
								<div class="detail-bottom">
									...
									<div v-if="num2==0" class="detail-public">
										...
										<div class="bottom-details">
											<li >
												<span v-if="cgxqInfo.logisticsNo">物流编号：{{cgxqInfo.logisticsNo}}</span>
											</li>
											<li>
												{{$t('orders.orderA')}} 
										...
										<!--底部的分页-->
										<div class="page-box" v-if='mxtotal!=0'>
											 <Page :total="mxtotal" size="small" show-total  :pageSize='mxpageSize'  @on-change="mxgetData" @on-page-size-change="mxchangePageSize"></Page>
										</div>
									</div>
									<!--记录-->
									<div v-if="num2==2" class="detail-public">
										<Timeline v-if='jldata.length!=0' style="height:300px;overflow:auto">
									        <TimelineItem v-for='(item,index) in jldata' :key='index' >
	...
	<Modal v-model="alertch" width="750px">
		...
		<div style="position:relative;text-align:center;width:100%;height:35px;line-height:35px;display:flex;">
			<Input v-model="gyskeyword":placeholder="$t('public.codeandname')" style="padding:2px 0" @on-enter='enterSearchS'></Input>
		...
		<div slot="footer">
			...
			<div class='title' style='margin:0px'>
				...
				<div>
					...
					<Button shape="circle" type="ghost" style="display:inline-block;background:#3b77e3;color:#fff" @click="makegys"><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>{{$t('public.sure')}}</Button>
				</div>
			</div>
		</div>
	</Modal>
	...
	<Modal v-model="warehouseshow" width="750px">
		...
		<div style="position:relative;text-align:center;width:100%;height:35px;line-height:35px;display:flex;">
			<Input v-model="ckkeyword"  :placeholder="$t('public.codeandname')" style="padding:2px 0" @on-enter='enterSearchw'></Input>
		...
		<div slot="footer">
			...
			@on-row-click="ckrow" @on-row-dblclick="dbckrow">
			</Table>
			<!--分页-->
            <div class="page-box" style="text-align:left">
			...
			<div class='title' style='margin:0px'>
				...
				<div>
					<Button shape="circle" type="ghost" style="display:inline-block;background:#ACACAC;border:none;color:#fff" @click='modelCancel'><Icon type="close" style="margin-right:5px;" ></Icon>{{$t('public.cancel')}}</Button>
					<Button shape="circle" type="ghost" style="display:inline-block;background:#3b77e3;color:#fff" @click="makeck"><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>{{$t('public.sure')}}</Button>
				</div>
			</div>
		</div>
	</Modal>
	...
	<Modal v-model="goodsinshow" width="750px">
		...
		<div style="position:relative;text-align:center;width:100%;height:35px;line-height:35px;display:flex;">
			<Input  :placeholder="$t('public.codeandname')" style="padding:2px 0" v-model="goodskeyword" @on-enter='enterSearchcg'></Input>
	...
	<Modal v-model="cgDZisshow" width="660px">
		<p slot="header" style="height:30px;line-height:30px">
			<span>{{$t('purchase.purchaseOrder.type1')}}</span><!--单证类型-->
		</p>
		...
		<div slot="footer">
			<div >
				<div>
					...
					<Button shape="circle" type="ghost" style="display:inline-block;background:#3b77e3;color:#fff;border:none" @click="cgtypeSureattr"><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>{{$t('public.sure')}}</Button>
				</div>
			</div>
		</div>
	</Modal>
	    ...
	    <p slot="header" style="height:30px;line-height:30px;text-align:center">
	     ...
	     <Form :label-width="100">
	       ...
	       <FormItem :label="$t('purchase.purchaseOrder.date1')"><!--收货日期-->
			<DatePicker type="date" :value='deliveryDateL' :placeholder="$t('purchase.purchaseOrder.date1')" style="width:100%" @on-change="shdateL"></DatePicker>
	    	</FormItem>
	    </Form>
	    <div slot="footer">
	...
	export default {
		...
		data() {
			return {
                ...
                importstore:[
                    {
						title: this.$t('purchase.purchaseOrder.cgsend.formcode'),//'来源编号',
						align: 'center',
						width:160,
					...
					{
						title: this.$t('purchase.purchaseOrder.cgsend.tocode'),//'收货编号',
						key: 'recordNo',
						width:160,
                ...
                warehouseName:[
                         { required: true, message: '仓库或者店铺不能为空',},
								...
								return h('InputNumber',{
										...
										on:{
											...
											"on-change":(e)=>{
													...
													this.cunhuodata[parmas.index].receiptQty=this.allCouunt
													// this.cunhuodata[parmas.index].receiptQty=parmas.row.qty
											...
											'on-blur':()=>{
													...
													this.cunhuodata[parmas.index].receiptQty=parseInt(this.allCouunt)
								...
								return h('InputNumber',{
									...
									on:{
										...
										"on-change":(e)=>{
											...
											parmas.row.allprice=Number(parmas.row.costPrice)*parmas.row.qty
											parmas.row.allprice.toFixed(2)
											parmas.row.amount=Number(parmas.row.costPrice)*parmas.row.qty
											parmas.row.amount.toFixed(2)
										},
										// 'on-blur':()=>{
										// 	let resultqty=0
												...
												click:(e)=>{
													e.stopPropagation()
														...
														if(this.cgListInfo.length==1){
															this.cgListInfo=[{skuName:this.$t('operation.yhqHome.choosesku'),allprice:0,qty:0,costPrice:0}]//'选择商品'
														}else{
															
															this.cgListInfo.splice(params.index,1)
								...
								return h('InputNumber',{
										...
										on:{
											...
											"on-change":(e)=>{
													...
													this.cgListInfo[parmas.index].qty=this.allCouunt
													this.cgListInfo[parmas.index].receiptQty=this.allCouunt
													// this.cgListInfo[parmas.index].receiptQty=parmas.row.qty
													parmas.row.allprice=Number(parmas.row.costPrice)*parseInt(this.allCouunt)
													parmas.row.amount=Number(parmas.row.costPrice)*parseInt(this.allCouunt)
													parmas.row.allprice.toFixed(2)
													parmas.row.amount.toFixed(2)
											...
											'on-blur':()=>{
													...
													this.cgListInfo[parmas.index].receiptQty=parseInt(this.allCouunt)
													this.cgListInfo[parmas.index].allprice=Number(parmas.row.costPrice)*parseInt(this.allCouunt)
													this.cgListInfo[parmas.index].amount=Number(parmas.row.costPrice)*parseInt(this.allCouunt)
													for(let i=0;i<this.cgListInfo.length;i++){
														resultqty+=parseInt(this.cgListInfo[i].qty)
														resultPrice+=parseInt(this.cgListInfo[i].allprice)
													}
												// this.totalCount=parseInt(resultqty)//总数量
												// this.totalAmount=resultPrice//总价值
												// this.totalAmount.toFixed(2)//总价值
								...
								return h('InputNumber',{
									...
									on:{
										...
										"on-change":(e)=>{
											...
											this.cgListInfo[parmas.index].costPrice=e
											parmas.row.allprice=Number(parmas.row.costPrice)*parmas.row.qty
											parmas.row.allprice.toFixed(2)
											parmas.row.amount=Number(parmas.row.costPrice)*parmas.row.qty
											parmas.row.amount.toFixed(2)
										...
										// 	this.cgListInfo[parmas.index].costPrice=Number(e)
										// 	this.cgListInfo[parmas.index].allprice=Number(parmas.row.costPrice)*Number(parmas.row.qty)
										// 	this.cgListInfo[parmas.index].amount=Number(parmas.row.costPrice)*Number(parmas.row.qty)
                ...
                jl:[
                	...
                	{
                		title:this.$t('purchase.purchaseOrder.date'),//单据日期
                		key:"poDate"
                	},
		...
		methods: {
						  ...
						  //       });
					 		// }else{
					 			this.axios.post('/receipt/add?uid=' + this.uid, {
					            	...
					            	// receiptQty:this.getAllqty(this.detailVoList),
			...
			//商品弹框点击一行获取数据
			clickgoodsrow(data){
				this.goodsrow = data//选中商品一行的数据GET /sku/get/{id} 详情
				...
				if(this.rowgoodsDataAll.length!=0){
					this.rowgoodsDataAll.forEach((x,index)=>{
						if(x.attrDetailList!=null && x.attrDetailList.length!=0){
							...
							let arr={
								skuName:this.this.rowgoodsDataAll[index].skuName,
								skuId:this.this.rowgoodsDataAll[index].id,
								inventory:this.this.rowgoodsDataAll[index].inventory,
								enterInventory:this.this.rowgoodsDataAll[index].enterInventory,
								qty:Number(this.allCouunt),
								price:this.this.rowgoodsDataAll[index].price,
								costPrice:this.this.rowgoodsDataAll[index].costPrice,
								skucode:this.this.rowgoodsDataAll[index].skuCode,
								barcode:this.this.rowgoodsDataAll[index].barcode,
								unit:this.this.rowgoodsDataAll[index].unit,
								unitId:this.this.rowgoodsDataAll[index].unitId,
								wholePrice:this.this.rowgoodsDataAll[index].wholePrice,
								unitList:this.this.rowgoodsDataAll[index].unitList,
								skuTypeId:"",//属性编号
								skuTypeDesc:"",//属性描述
								skuPic:this.this.rowgoodsDataAll[index].mainPhoto,
								receiptQty:Number(this.allCouunt),
								zoneNo:"",
								zoneId:"",
								locationNo:"",
			...
			remove (index) {
				...
				this.cgListInfo.splice(index, 1);
			 ...
			 this.$refs[name].validate((valid) => {
			 	if(valid){
			 		if(this.cgListInfo[0].skuName=="选择商品"){
			 			...
			 			this.axios.put('receipt/update?uid='+this.uid,{
							id:this.id,
							type:this.type,
							address:this.address,//发货人地址
	            			...
	            			detailVoList:this.cgListInfo,//采购明细
	            			//仓库
		            		deliverWarehouseName:this.formValidate.warehouseName,//仓库名
		            		deliverWarehouseCode:this.warehouseCode,//仓库编号
		            		deliverWarehouseId:this.warehouseId,//仓库名id
			...
			getDefaultAnddetails(id){
				...
				//获取采购订单详情上方
				this.axios.get('/receipt/get/'+id+'?uid='+this.uid).then((res)=>{

					if(res.data.status==200){
						let data=res.data.rows
						// this.type=data.type

						this.address=data.address//发货人地址
						this.deliverName=data.deliverName//收货方名称
            			...
            			//仓库
	            		this.formValidate.warehouseName=data.deliverWarehouseName//仓库名
	            		this.warehouseCode=data.deliverWarehouseCode//仓库编号
	            		this.warehouseId=data.deliverWarehouseId//仓库名id

	            		this.formValidate.supplierName=data.supplierName//供应商名称
		            	this.supplierId=data.supplierId//供应商id
						this.recordNo=data.recordNo//单号
						// totalQty:this.totalQty,//采购数量
						this.deliveryQty=data.deliveryQty//已发运数量
						this.logistics=data.logistics//物流公司
						this.logisticsNo=data.logisticsNo//物流编号
						
						this.poDate=data.poDate//订单日期
				...
				//获取采购订单明细下方
				this.axios.get('/receipt/detail/'+id+'?uid='+this.uid).then((res)=>{
					if(res.data.status==200){
						if(res.data.rows.length==0){
							this.cgListInfo=[{skuName:this.$t('operation.yhqHome.choosesku'),allprice:0,qty:0,costPrice:0}]//'选择商品'
							this.cunhuodata=[{skuName:this.$t('operation.yhqHome.choosesku'),allprice:0,qty:0,costPrice:0}]//'选择商品'
						}else{
							...
							this.cgListInfo.forEach((item)=>{
								totalP = item.costPrice*item.qty
								item['totalP'] = totalP//添加一个属性总价
								
							})
							this.cunhuodata=res.data.rows//收货商品明细
							this.cunhuodata.forEach((x)=>{
								if(x.qty==null){
									return
								}else{
									x.allprice=x.qty*x.costPrice
									x.poDetailId=x.id
								}
			...
			getAllqty(arr){
			 ...
			 //明细切换一条页
            mxgetData(current){
                this.axios.get('receipt/detail?offset='+current+'&uid='+this.uid).then((res)=>{
              		if(res.data.status==200){
						this.cgListInfo = res.data.rows
						this.mxtotal=res.data.total
						this.mxpageSize=res.data.pageSize
			...
			delcg(){
				if(this.id==''){
					...
					this.axios.delete('/receipt/delete/'+this.id+'?uid='+this.uid).then((res)=>{
						if(res.data.status==200){
							this.$notify({
								title:"",//
			...
			//审核订单
			examine(){
				this.axios.get('receipt/submit/'+this.id+'?uid='+this.uid).then((res)=>{
					if(res.status==200){
						this.$notify({
						        title: '审核',
						        message: '已审核',
			...
			//弃审
			cancel(){
				this.axios.get('receipt/cancel/'+this.id+'?uid='+this.uid).then((res)=>{
					if(res.status==200){
            			this.$notify({
					        title: '弃审',
					        message: '已弃审',
			...
			//作废
			invalid(id){
				this.axios.get('receipt/invalid/'+this.id+'?uid='+this.uid).then((res)=>{
					if(res.data.status==200){
						this.$notify({
					        title: '作废',
					        message: '已作废',
			...
			//收货完成
			allsign(){
				this.axios.get('receipt/confirm/'+this.id+'?uid='+this.uid).then((res)=>{
					if(res.data.status==200){
						this.$notify({
					        title: '',
					        message: '已收货完成',
			...
			//逐条签收
			onebyone(){
				this.axios.delete('receipt/sign?id='+this.id+'&uid='+this.uid).then((res)=>{
			...
			//物流登记
			Logistics(){
				this.axios.put('/receipt/logistics?'+this.id+'&logistics='+this.logistics+'&logisticsNo='+this.logisticsNo+'&uid='+this.uid).then((res)=>{
					if(res.data.status==200){
			...
			//仓库模糊搜索
			getckKeyword(){
				this.axios.get('warehouse/query?keyword='+this.ckkeyword+'&uid='+this.uid+'&status=1').then((res)=>{
					if(res.data.status==200){
						this.allwarehouse = res.data.rows
					}
				})
			...
			addborder(index){
			 ...
			 //记录切换一条页
			 getReceipt(id){
			 	this.axios.get('receipt/log?id='+id+'&uid='+this.uid).then((res)=>{
			 		this.jldata = res.data.rows
					this.jl1total=res.data.total
					this.jl1pageSize=res.data.pageSize
					this.jldata.forEach((x)=>{
			...
			//保存后获取全部收获i订单
			getcgListS() {
				this.axios.get('/receipt/query?uid=' + this.uid+'&status=1').then((res) => {
					let data = res.data;
					if(res.data.status == '200') {
						this.Allpo = data.rows;
						this.pototal=data.total
            ...
            handleReachEdge(){
				this.current++//默认10条
    	 		this.axios.get('/receipt/query?offset='+this.current+'&uid='+this.uid+'&status='+this.valueT).then((res)=>{
           ...
           //获取单证类型
			...
			orderselected(value){
				this.valueT = value
				this.axios.get('/receipt/query?uid=' + this.uid+'&status='+this.valueT).then((res) => {
					if(res.data.status == '200') {
            ...
            //精准搜索确认
            sureSearchLocation(){
               this.axios.get('/receipt/query?uid=' + this.uid,{
           ...
           clickreservoir(item){
           		this.goodsItem=item
           		console.log(this.cgListInfo[this.goodsItem.index].locationNo)
           ...
           locationSure(){
	           	...
	           	if(arr.length==0 || this.kqSing==''){
					...
					this.locationId=this.locationObj.id//库位IDcunhuodata
		           	this.reservoirisshow=false

		           	this.cgListInfo[this.goodsItem.index].locationNo=this.locationName
		           	this.cgListInfo[this.goodsItem.index].locationId=this.locationId
		           	this.cgListInfo[this.goodsItem.index].zoneId=this.reservoirId
					this.cgListInfo[this.goodsItem.index].zoneNo=this.reservoirName
		           		console.log(this.cgListInfo)
		           	this.detailVoList[this.goodsItem.index-1].locationNo=this.locationName
		           	this.detailVoList[this.goodsItem.index-1].locationId=this.locationId

		           	this.detailVoList[this.goodsItem.index-1].zoneId=this.reservoirId
					this.detailVoList[this.goodsItem.index-1].zoneNo=this.reservoirName
				
					this.cunhuodata[this.goodsItem.index].locationNo=this.locationName
		           	this.cunhuodata[this.goodsItem.index].locationId=this.locationId
		           	this.cunhuodata[this.goodsItem.index].zoneId=this.reservoirId
					this.cunhuodata[this.goodsItem.index].zoneNo=this.reservoirName
           ...
           //打印GET /receipt/export 导出
		 printOrder(){
				this.axios.get('receipt/export?ids='+this.orderStr+'&uid='+this.uid).then(res=>{
					if(res.data!=null){
						this.$notify({
							title:"",//
							message: res.data.errorMessage,
