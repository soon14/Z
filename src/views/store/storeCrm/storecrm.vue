			...
			<div style='height:100%;width: 100%;' >
				<Tabs :animated="false" style="background:#fff;width:100%;height:100%" @on-click="getfhlistbg">
			        	...
			        	<div class="table">
							<div style='margin:10px' class="AllMain">
								<div class="inputSearchAll">
									<Input class="searchGoods" style='margin-top:10px;width:80%; margin-left: 10px;' v-model='kckeywordAll' placeholder='请输入会员编号/名称/手机号' @on-enter='entersSearchkcAll' >
				            		</Input>
				            		<Button @click='searchkcAll' size="large" type="primary" icon="ios-search" style="margin:8px 0 0 10px;">搜索</Button>
			            		</div>
			            		<div v-if='loadingisshow' style="margin-top:40px">
									<Spin>
			        ...
			        <TabPane label="会员中心">
			        	<!--会员清单-->
							<div class="table">
								<div style='height:50px;width:100%;line-height:50px;display:flex;justify-content: space-between;'>
									<div style='display:flex'>
										<Input style='margin-top:10px;width:300px' v-model='jtkeyword' :placeholder="$t('public.nameandphone')" @on-enter='entersSearchJtCrm' >
								...
								<div class='cradList'>
									...
									<div class='cradList1' v-for='(item,index) in cardData' :key='index' v-else>
										...
										<div class='viptopmain-bottom'>
											...
											<div style='font-size:25px;font-weight:900;color:#E1A271'>{{item.useTypeDesc}}</div>
										...
										<div class='viptopmain-b'>
											...
											<div>
												<li class='remarkli'>
													<span class='remarkli-span'>开卡押金</span>
													<span class='twospan'>{{item.deposit}}元</span>
												</li>
												<li class='remarkli'>
													<span class='remarkli-span'>开卡限制</span>
													<span class='twospan'>满{{item.consumptionAmount}}元 赠送{{item.giftIntegral}}积分</span>
												</li>
												
												<li class='remarkli'>
													<span class='remarkli-span'>升级规则</span>
													<span class='twospan'>累计消费{{item.cumulativeConsumptionAmount}}元</span>

													<span style="margin-left:5px">累计获得{{item.cumulativeIntegral}}积分</span>
												</li>
												
												<li class='remarkli'>
													<span class='remarkli-span'>是否默认</span>
													<span class='twospan'>{{item.defaultValue==true?"是":"否"}}</span>
												</li>
												<li class='remarkli'>
													<span class='remarkli-span'>有效期限</span>
													<span class='twospan'>{{item.timeMonth==0?"永久有效":item.timeMonth+'个月'}}</span>
												</li>
												<li class='remarkli'>
													<span class='remarkli-span'>权限说明</span>
													<span class='twospan'>{{item.roleMemo}}</span>
												</li>
											</div>
										</div>
									</div>
			        ...
			        <TabPane label="充值活动">
						...
						<div class="table">
							<div style='display:flex'>
								<Input style='margin: -1px 0 10px 0px;;width:300px' v-model='mrechargeakeyword' :placeholder="$t('public.nameandphone')" @on-enter='entersSearchmrechargea' >
			        ...
			        <TabPane label="近期生日">
						...
						<div class="table">
								<div style='display:flex;margin: 10px 0 10px 0;'>
									<Select v-model="valueT" style="width:100px;margin-right:10px" @on-change="changeBirthday">
								        <Option v-for="item in BList" :value="item.value" :key="item.value">{{ item.label }}</Option>
								    </Select>
									<Input style='width:400px' v-model='birthdaykeyword' :placeholder="$t('public.nameandphone')" @on-enter='entersSearbirthday' >
        		...
        		<Tabs @on-click="clickmsg" v-model="numIndexmsg" :animated="false">
			        ...
			        <div >
						...
						<div  style='display:inline-block'>
							...
							<div class="list-box list-box1">
							<span>性别 : </span><div>{{detailsData.gender=='0'?"女":detailsData.gender=='1'?'男':""}}</div>
							</div>
							<div class="list-box list-box1">
      ...
      lunxun(num){
        this.axios.get('/mrecharge/searchby/'+num+'?uid='+this.uid).then(res=>{
	      ...
	      //获取活动数据GET /mrechargea/query 充值活动清单
            ...
            //订单
            getorderCrm(){
            	this.axios.get('orderon/crmOrder/'+this.detailsData.id+'?uid='+this.uid).then((res)=>{
                    //console.log(res)
            ...
            ordergetData(current){
                this.axios.get('orderon/crmOrder/'+this.detailsData.id+'?offset='+current+'&uid='+this.uid).then((res)=>{
                    //console.log(res)
                    this.orderdataList = res.data.rows
                    this.ordertotal=res.data.total;
					this.orderpageSize = res.data.pageSize
