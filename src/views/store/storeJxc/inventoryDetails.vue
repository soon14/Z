...
export default{
	...
	methods:{
		search(){
			this.axios.get('minventory/listDetail?keyword='+this.searchKeyword+'&uid='+this.uid).then(res=>{
        ...
        handleReachEdge(){
			this.current++//默认10条
	 		this.axios.get('minventory/query?offset='+this.current+'&uid='+this.uid).then((res)=>{
