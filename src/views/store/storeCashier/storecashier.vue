<template>
	...
	<!--左侧-model='employeeId' -->
							...
							<li v-for='(item,index) in yhqData' :key='index' class='gdli gdli1'
							 @click='clickgdli1(item,index)' :class="{gdlibg1:index==gdliindex1}">
								...
								<div class='gdlidiv'>
									...
									<span>
										<span>
										{{item.type==3?"折扣：":item.type==1?"减免：":item.type==2?"减：":item.type==4?"送：":""}}
										</span>
							...
							<li v-for='(item,index) in Allgd' :key='index' class='gdli' @click='clickgdli(index,item)' :class="{gdlibg:index==gdliindex}" >
								<div class='gdlidiv'>
									...
									<div style='display:flex'>
										...
										<span style='color:#E1A271;margin-top:5px'>{{NumAll(item.details)}}</span>
									</div>
								</div>
								<div class='gdlidiv'>
									<div style='display:flex'>
		...
		<div class='right-content'>
			...
			<div style='width:100%;height:1px;background: rgb(230,233,236);'></div>
			<div class="table">
				...
				<div class='bottom-mark'>
					<div style='display:flex;width: 50%;text-align: center;justify-content: center;flex-direction: column;'>
						...
						<div>
							...
							<li>
								<span>商品数量：<span style='color:#D76969'>{{NumAll(this.dataList)}}</span></span>
								<span style="margin-left:20px">{{yhqItem.type==1?"卡券减免：":yhqItem.type==3?"卡券折扣：":"卡券减免："}}<span style='color:#D76969'>{{yhqItemamount}}{{yhqItem.type==1?"元":yhqItem.type==3?"折":"元"}}</span></span>
							</li>
						</div>
					</div>
	...
	<!--跨点发货弹框kdisshow-->
	<Modal v-model="kdisshow" width="600px" >
...
<script type="text/javascript">
	export default{
		...
		methods:{
			...
			oneStep(){
				...
				this.detalistPay=[]//每点击一次虚清空//防误点
					...
					this.axios.post('morder/sel?uid='+this.uid,{
						...
						chargeAmount:this.crmDetails.chargeMoney,//余额
						employeeId:this.employeeId,
						employeeName:this.employeeName,
			...
			changeCode(s){
				if(this.fkmcode.length==18 &&this.fkmcode.length!=''){
				...
				this.axios.get('setting/dqmddg?uid='+this.uid).then((res)=>{
            		if(res.data.status==200){
						...
						data.forEach((item,index)=>{
							...
							this.employeeId=this.employeeVos[0].value
						})
				...
				if(arr.length==0){
					...
					if(this.yhqItem.endTime!=null){
						if(yhqEndTime<date){
							...
							this.yhqItemamount=this.yhqItem.amount
