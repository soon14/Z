...
<template>
	...
	<div class="add-box">
		...
		<div class="head">
				...
				<div class="right-top">
				<div >
					<span style="width: 200px;margin-left:20px;font-size:19px;font-weight:900;position: absolute;left:360px;">{{addtx}}</span>
					<!--<div v-if="bordercodeisshow" style="margin-top:-20px">{{poId}}</div>条码编号-->
				</div>
				<div style="position: absolute;right: 240px;height: 60px;text-align: right;">
					<div v-if="addText">
						<div style='width:95px;text-align: center;right: 185px;display:inline-block;position:absolute;top:14px;height:35px;line-height:35px;overflow:hidden;z-index-1' >
			                  <el-upload
			                    class="upload-demo"
			                    :action="actionUrl"
			                    :on-success='success'
						...
						<Button shape="circle"  style="background:#3b77e3;color:#fff;border:none" @click="saveCgInfo('xiaoshousave')" >
						<Icon type="checkmark-round" style="margin-right:5px;color:#fff"></Icon>保存</Button>

						<Button shape="circle"  @click="showDetail"  style="background:#999999;color:#fff;" ><Icon type="close" style="margin-right:5px;"></Icon>取消</Button>
					...
					<div v-if="!addText" class="changebutton" style='position:relative'>
						<div style='width:140px;right: 183px;display:inline-block;height:32px;line-height:32px;' v-if='fileBtn'>
			                  <el-upload
			                    class="upload-demo"
			                    :action="actionUrl"
			                    :on-success='success'
						...
						<Button type="ghost"  shape="circle"  @click="cancel" v-if='qs' style="background:#d53c39;color:#fff;border:none">
							<Icon type="android-cancel" style="position:relative;right:5px"></Icon>
						弃审
						</Button>
						<Button type="ghost"  shape="circle"  @click="deliveryClick" v-if='deliveryshow' style="background:#3b77e3;color:#fff;border:none">
							<Icon type="checkmark-round" style="position:relative;right:5px"></Icon>
						发货
						</Button>
						<Button type="ghost"  shape="circle"  @click="deliverysave" v-if='deliveryshowsave' style="background:#3b77e3;color:#fff;border:none">
							<Icon type="checkmark-round" style="position:relative;right:5px"></Icon>
						保存
						</Button>
						...
						<!-- <Button type="ghost"  shape="circle"  @click="package" v-if='packageshow' style="background:#3b77e3;margin-right:5px;color:#fff;border:none">
							<Icon type="checkmark-round" style="position:relative;right:5px"></Icon>

						打包
						</Button> -->
						<!-- <Button type="ghost"  shape="circle"  @click="shipment" v-if='shipmentshow' style="background:#3b77e3;margin-right:5px;color:#fff;border:none">
							<Icon type="checkmark-round" style="position:relative;right:5px"></Icon>
						...
						<Button type="ghost"  shape="circle" @click="saveEdit('xiaoshousave1')" v-if="editshow" style="background:#3b77e3;color:#fff;border:none">
							<Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>保存
						</Button>
						<Button shape="circle" v-if="editshow" ref='editquxiao'  @click="showDetail"  style="background:#999999;color:#fff;" ><Icon type="close"  style="margin-right: 6px"></Icon>取消</Button>
						<Dropdown trigger="click" >
					        ...
					        <DropdownMenu slot="list" style='text-align:left'>
					            ...
					            <span @click="addPurchase" v-if="turnPurchase"><DropdownItem>转采</DropdownItem></span>
	...
	<div class="left-content">
		...
		<Scroll :on-reach-bottom="handleReachEdge" :height="height" v-else >
			<div class="con-list" v-for="(item,index) in Allpo" :key='index' @click="getcginfo(item.id,index,item)" :class="{bg:numIndex==index}">
				...
				<div class="lanrenLeft" :class="{bgd6:item.status==1,bgd7:item.status==99,bgd8:item.status==0,bgd9:item.status==88,bgd10:item.status==3,bgd11:item.status==8}">
	...
	<div class="add-right">
		<div class="right-content" ref="divsave">
			<!--销售保存-->
			<div v-if="addText" style='margin-bottom:100px;width:100%;padding:10px'>
						...
						<Col span="8">
	                   	...
	                   	<Col span="8">	
	                   		<FormItem label="销售类型" prop="salesType">
	                        	...
	                        	<Select filterable v-model="xiaoshouformValidate.salesType" @on-change="getcgType" @on-query-change="changeSearch" label-in-value class='sele'>
	                   	...
	                   	<Col span="8">
	                   		<FormItem label="支付方式" prop="modeldztype">
	                        	<Select filterable v-model="xiaoshouformValidate.modeldztype" @on-change="getdztype">
				...
				<div style='text-align:right;height:35px;border:1px solid #e4e4e4;line-height:35px;border-top:none'>
					<span style='margin-right:20px'>
						...
						<a>{{getAllqty(cunhuodata)}}</a>
					 	...
					 	共：
					 	<span style="color:#ff0000">￥：{{amountAll(cunhuodata)}} 元</span>
					</span>
				</div>
			</div>
			<div style='margin: 10px 0px 0px 0px;display:flex'>
				<div style='margin:0 10px;display:flex'>
					<div style='margin:10px 0'><span>总金额：</span><span style="color:#ff0000">{{amountAll(cunhuodata)}}</span><span>元</span></div>
				
				</div>
			
				<div style='margin:0 10px;display:flex'>
					<div style='margin:10px 0'><span>已收金额：</span></div>
				...
				<div style='margin:0 10px;display:flex'>
					<div style='margin:10px 0'><span>应收金额：</span><span style="color:#ff0000">{{amountAll(cunhuodata)-discountAmount-payAmount}}</span><span>元</span></div>
					
				</div>
			</div>
			<div style='margin:10px 0'>
				<div style='margin:10px'><span>备注：</span></div>
	...
	<!--发货-->
	<div class="right-content" ref="fh">
		<div v-if="fh" style='margin-bottom:100px;width:100%;padding:10px'>
				...
				<Form  :label-width="80" >
					<Row  style='margin-top: 10px;'>
						<Col span="8">
							...
							<FormItem label="客户" >
	                        	<Input  v-model="xiaoshouformValidate.partnerName" placeholder="请选择客户" disabled></Input>
	                   	...
	                   	<Col span="8">
	                   		<FormItem label="选择仓库" >
	                        	<span  @click="warehouseModal">
									<Input v-model="warehouseName" :placeholder="$t('purchase.purchaseOrder.ck')"></Input>
								</span>
	                   		</FormItem>
	                   	</Col>
	                   	<Col span="8">
	                   		<FormItem label="单据日期" prop="recordDate">
	                        	<DatePicker type="date" :value='recordDate' placeholder="请选择时间" style="width:100%" @on-change="shdate1"></DatePicker>
	                   		</FormItem>
			...
			<!--选择商品表格-->
			<div class="table">
				<Table size="small" :highlight-row="true" border :columns="cunhuo2" :data="cgListInfo" ></Table>
				<div style='text-align:right;height:35px;border:1px solid #e4e4e4;line-height:35px;border-top:none'>
					<span style='margin-right:20px'>
						{{$t('orders.orderA')}} 
						<a>{{getAllqty(cgListInfo)}}</a>
					 	{{$t('orders.orderAjian')}}；
					 	
					 	共：
					 	<span style="color:#ff0000">￥：{{amountAll(cgListInfo)}} 元</span>
					</span>
				</div>
			</div>
			<div style='margin:10px 0'>
				<div style='margin:10px 0'><span>备注：</span></div>
				<Input v-model="cgxqInfo.remark" style="width:100%" type="textarea" placeholder="备注信息 :" class="getaddress" :min-row="6"></Input>
			</div>
			<div  v-if='soe'>
	            <Spin>
	                <Icon type="load-c" size=18 class="demo-spin-icon-load"></Icon>
	...
	<div class="right-content" ref="edit">
		<!--修改-->
		<div v-if="editxiugai" style='padding:10px'>
					...
					<Row  style='margin-top: 10px'>
						<Col span="8">
	                   	...
	                   	<Col span="8">	
	                   		<FormItem label="销售类型" prop="salesType">
	                        	...
	                        	<Select @on-change="getcgType" filterable label-in-value v-model='xiaoshouformValidate.salesType' class='sele'>
	                        		<div class='optionMain'>
										<Option v-for="(item,index) in datacgList" :value='item.value' :key='item.value'>{{item.label}}</Option>
									</div>
								</Select>
						...
						<div class="table">
							...
							<div style='text-align:right;height:35px;border:1px solid #e4e4e4;line-height:35px;border-top:none'>
								<span style='margin-right:20px'>
									{{$t('orders.orderA')}} 
									<a>{{getAllqty(soDetailList)}}</a>
								 	{{$t('orders.orderAjian')}}；
								 	
								 	共：
								 	<span style='color:red'>￥：{{amountAll(soDetailList)}} 元</span>
								</span>
							</div>
						</div>
						<div style='margin: 10px 0px 0px 0px;display:flex'>
							<div style='margin:0 10px;display:flex'>
								<div style='margin:10px 0'><span>总金额：</span><span style="color:#ff0000">{{amountAll(soDetailList)}}</span><span>元</span></div>
							...
							<div style='margin:0 10px;display:flex'>
								<div style='margin:10px 0'><span>应收金额：</span><span style="color:#ff0000">{{amountAll(soDetailList)-discountAmount-payAmount}}</span><span>元</span></div>
							</div>
						</div>
				</Form>
				<div style='margin:10px 0'>
	...
	<div class="right-content right-detail" ref="detail" style="display:none;">
			<el-steps :active="activeSend" finish-status="success" :align-center='true' style='margin-left:80px;' v-if='activeIsShow' class="fatherStep
			 ">
				<el-step title="新增"></el-step>
				<el-step title="审核"></el-step>
				<el-step title="发货"></el-step>
						...
						<Col span="8">
	                   	...
	                   	<Col span="8">	
	                   		<FormItem label="销售类型" prop="salesType">
	                        	...
	                        	<Select disabled  label-in-value v-model='xiaoshouformValidate.salesType' class='sele'>
	                        		<div class='optionMain'>
										<Option v-for="(item,index) in datacgList" :value='item.value' :key='item.value'>{{item.label}}</Option>
									</div>
								</Select>
	                   	...
	                   	<Col span="8">
	                   		<FormItem label="单据日期" prop="recordDate">
	                        	<DatePicker disabled type="date" :value='xiaoshouformValidate.recordDate' placeholder="请选择时间" style="width:100%" ></DatePicker>
	                   		</FormItem>
	                   	</Col>
	                   	<Col span="8">
	                   		<FormItem label="到货日期">
	                        	<DatePicker disabled type="date" :value='deliveryDate' placeholder="请选择时间" style="width:100%"></DatePicker>
	                   		</FormItem>
						</Col>
						</Row>
						<Row  style='margin-top: 10px;'>
						<Col span="8">
							<FormItem label="收货人" prop="deliverName">
								<div  style='position:relative;'>
	                        		...
	                        		<Icon type="ios-search" size="16" style='position:absolute;top:8px;right:8px;cursor:pointer' ></Icon></span>
	                        	</div>
		                    </FormItem>
			              </Col>
			             <Col span="8" >
			                 ...
			                 <Col span="16">
			                     <FormItem label="收货方地址" >
									<div style='display:flex'>
									  	...
									  	<Input disabled type="text" v-model="deliverAddress" placeholder="请输入详细地址" style="margin-left:10px"></Input>
		           					</div>
			                    </FormItem>
							</Col>
			...
			<!--**********下方数据***********-->
			<!--状态-->
			<!-- <div class="lanren" :class="{bgd1:cgxqInfo.ststusDesc=='未审核',bgd2:cgxqInfo.ststusDesc=='已作废',bgd3:cgxqInfo.ststusDesc=='待审核',bgd4:cgxqInfo.ststusDesc=='已完成',bgd5:cgxqInfo.ststusDesc=='已审核',bgd61:cgxqInfo.ststusDesc=='打包中'}">
	           <span  >{{cgxqInfo.ststusDesc}}</span>
	        </div>​ -->
			...
			<div class="detail-bottom">
				...
				<div v-if="num2==0" class="detail-public" >
					...
					<div style='text-align:right;height:35px;border:1px solid #e4e4e4;line-height:35px;border-top:none;display:flex;justify-content: space-between;'>
						<div style='margin:0 0px 0px 5px;display:flex'>
							<div style='margin:0 10px;display:flex'>
								总金额：<span style="color:#ff0000">{{totalAmount}}</span>元
							</div>
							<div style='margin:0 10px;display:flex'>
								已收金额：<span style="color:#ff0000">{{payAmount}}</span>元
							</div>
							<div style='margin:0 10px;display:flex'>
								优惠金额：<span style="color:#ff0000">{{discountAmount}}</span>元
							</div>
							<div style='margin:0 10px;display:flex'>
								应收金额：<span style="color:#ff0000">{{receivedAmount}}</span>元
							</div>
						</div>
						<span style='margin-right:20px'>
							{{$t('orders.orderA')}} 
					...
					<Page v-if='total1!=0' size="small" :total="total1" show-total  :pageSize='pageSize1'  @on-change="getData1" @on-page-size-change="changePageSize1"></Page>
					</div>
				</div>
			</div>
			<!--销售发货清单-->
			<div v-if="num2==1" class="detail-public">
				<Table size="small" border  :columns="deliveryListkey" :data="deliveryListData"></Table>
				<div class="page-box" v-if='deliveryListtotal>0' style='margin-bottom:120px'>
					 <Page :total="deliveryListtotal" show-total size="small" :pageSize='deliveryListpageSize'  @on-change="deliveryListgetData" @on-page-size-change="deliveryListchangePageSize"></Page>
				</div>
			</div>
				<!--日志-->
	...
	<Modal v-model="goodsinshow" width="760px">
		...
		<div slot="footer" style='margin-top:-10px'>
			...
			<div class='title' style='margin:0px'>
				...
				<div>
					<Button shape="circle" type="ghost" style="display:inline-block;background:#ACACAC;border:none;color:#fff" @click='modelCancelGoods'><Icon type="close" style="margin-right:5px;" ></Icon>取消</Button>
...
<!--商品属性弹框-->
	<Modal v-model="attrModel" width="460px" >
		...
		<div class="table1">
			...
			<Table  border  height='300' highlight-row  :columns="sAttrskey" :data="sAttrsData"
			 @on-row-click="clickAttrRow" @on-select='mSelected' @on-select-cancel='mcancel' @on-select-all='mselectAll' @on-selection-change="mchangeSelect"></Table>
		</div>
    ...
    <!-- <Modal v-model="pkisshow" width="420px">
    	...
    	<FormItem label="发运方式：" prop="model2">
        	<Select v-model="logisticsformValidate.model2" @on-change="getlogistics1" >
	...
	export default {
		...
		data() {
			return {
				...
				statusD:'',
				fh:false,//发货页面
				deliveryshowsave:false,
				editxiugai:false,//修改页面
				shyModeData:{},//弹框客户一行数据
				...
				xiaoshouruleValidate:{
					...
					deliverName:[{ required: true, message: '请输入收货人名称！'}, ],
					deliverMobile:[
					...
					// modeldztype:[{ required: true, message: '请选择支付方式！', trigger: 'blur' }, ],
					// salesType:[{ required: true, message: '请选择销售类型！', trigger: 'blur' }, ],
					// recordDate:[{ required: true, message: '请选择单据日期！', trigger: 'blur' }, ],
				},
				//收货人
				shrtotal:0,
				shrpageSize:0,
				...
				cgListInfoKey:[
					...
					},
					{
						...
						render:(h,parmas)=>{
							...
							},Number(parmas.row.salesPrice).toFixed(2))
						}
					},
					{
						title:"单位",
				...
				cunhuo2:[
									...
									h('img', {
										...
										on:{
											click:(e)=>{
												e.stopPropagation()
												//删除行
													if(this.cgListInfo.length==1){
														this.cgListInfo=[{skuName:"选择商品",amount:0,qty:0,salesPrice:"",allprice:""}]//'选择商品'
													}else{
														this.cgListInfo.splice(parmas.index,1)
													}
							...
							return h('InputNumber',{
									...
									on:{
										...
										"on-change":(e)=>{
											this.allCouunt = e
											parmas.row.qty= e
											this.cgListInfo[parmas.index]=parmas.row
											this.cgListInfo[parmas.index].qty=e
											...
											this.totalAmount=parmas.row.amount
										},
										'on-blur':()=>{
												let resultqty=0
												let resultPrice=0
												
												this.cgListInfo[parmas.index].qty=parseInt(this.allCouunt)
												this.cgListInfo[parmas.index].allprice=Number(parmas.row.salesPrice)*parseInt(Number(this.allCouunt))
												this.cgListInfo[parmas.index].amount=Number(parmas.row.salesPrice)*parseInt(Number(this.allCouunt))
											}
										}
									})
						}
					...
					},
					{
						...
						render:(h,parmas)=>{
							...
							},Number(parmas.row.salesPrice)*Number(parmas.row.qty).toFixed(2))
						}
					},
					{
						title:"属性",
								...
								return h('InputNumber',{
										...
										on:{
											...
											'on-blur':()=>{
													...
													this.cunhuodata[parmas.index].allprice=Number(parmas.row.salesPrice)*parseInt(Number(this.allCouunt))
													this.cunhuodata[parmas.index].amount=Number(parmas.row.salesPrice)*parseInt(Number(this.allCouunt))
													// ////////console.log(this.soDetailList)
												...
												// this.totalCount=parseInt(Number(resultqty))//总数量
				...
				cunhuoE:[
						...
						},
						{
							...
							render:(h,parmas)=>{
								...
								},parmas.row.amount.toFixed(2))//(Number(parmas.row.salesPrice))*(Number(parmas.row.qty))
							}
						},
						{
							title:"属性",
		     ...
		     //            },
                ...
                deliveryListkey:[
					...
					},
					{
						...
						render:(h,params)=>{
							return h('div',{
								style:{
									color:params.row.status==1?'#3b77e3':params.row.status==3?'#278fa3':params.row.status==99?'#d53c39':params.row.status==88?'#40ca98':params.row.status==0?'#3b77e3':params.row.ststusDesc=='打包中'?'#278fa3':''
								}
                ...
                deliveryListData:[],
                deliveryListtotal:0,
                deliveryListpageSize:0,
                //记录数据
                jldata:[],
                titles:["明细","发货清单","附件列表","日志",],
                id:"",//销售单id
				userList:[],//联系人列表
				Allpo: [], //采购列表
				cgListInfo:[],//采购订单明细列表
		...
		methods: {
			...
			copyadd(){
				...
				this.editshow = false
				this.fileBtn = false
				this.editxiugai=false//修改页面
				this.$refs.divsave.style.display="block"
				this.$refs.detail.style.display="none"
				this.$refs.edit.style.display="none"
				this.$refs.fh.style.display="none"
				this.xiaoshouformValidate.salesType=this.cgxqInfo.salesType//销售类型
        		this.xiaoshouformValidate.partnerName=this.cgxqInfo.partnerName//供应商名称
            	this.partnerId=this.cgxqInfo.partnerId//供应商id
				this.tel=this.cgxqInfo.tel
				this.phone=this.cgxqInfo.phone
				this.person=this.cgxqInfo.person
				this.fax=this.cgxqInfo.fax
  				this.email=this.cgxqInfo.email
  				this.address=this.cgxqInfo.address
  				this.xiaoshouformValidate.modeldztype=this.cgxqInfo.payType//支付方式
  				this.xiaoshouformValidate.deliverName=this.cgxqInfo.deliverName
  				this.xiaoshouformValidate.deliverTel=this.cgxqInfo.deliverTel
  				this.xiaoshouformValidate.deliverMobile=this.cgxqInfo.deliverMobile
  				this.deliverAddress=this.cgxqInfo.deliverAddress
  				this.deliverProvince=this.cgxqInfo.deliverProvince
  				this.deliverCity=this.cgxqInfo.deliverCity
  				this.deliverCountry=this.cgxqInfo.deliverCountry
				this.cunhuodata=this.cunhuodata.concat(this.cgxqInfo.soDetailList)//采购明细
				this.receivedAmount=this.cgxqInfo.receivedAmount//应付金额
				this.payAmount=this.cgxqInfo.payAmount//已收金额
				this.discountAmount=this.cgxqInfo.discountAmount//优惠金额
				this.xiaoshouformValidate.recordDate=this.cgxqInfo.recordDate//订单日期
				this.recordNo=this.cgxqInfo.recordNo
				this.totalAmount=this.cgxqInfo.totalAmount
				this.totalCount=this.cgxqInfo.totalCount
				this.soDetailList.length=this.cgxqInfo.soDetailList.length
				this.deliveryDate=this.cgxqInfo.deliveryDate
				this.remark=this.cgxqInfo.remark
				this.attridMore=this.cgxqInfo.attachs//多附件地址
        		...
        		if(this.deliverProvince==null || this.deliverProvince=='' || this.deliverCity==null || this.deliverCity=='' || this.deliverCountry==null || this.deliverCountry){
        			...
        			this.selectedOptions2=[this.TextToCode[this.deliverProvince].code,this.TextToCode[this.deliverProvince][this.deliverCity].code,this.TextToCode[this.deliverProvince][this.deliverCity][this.deliverCountry].code]
        		}
			},
			//双击获取订单详情
			getcginfo(id,index,item){
				...
				this.$refs.fh.style.display="none"
				this.soDetailList=[]
				this.cgListInfo=[]//重置收货/商品明细
				this.cunhuodata=[{skuName:'选择商品',allprice:0,qty:0,salesPrice:0}]
				this.getshjl(id)//日志
				this.getdeliveryListgetData()//发货清单
			...
			getDefaultAnddetails(id){
				...
				this.cgListInfo=[]//重置收货/商品明细
				//获取采购订单详情上方
				this.axios.get('/so/get/'+id+'?uid='+this.uid).then((res)=>{
					if(res.data.status==200){
						let data=res.data.rows
						this.cgxqInfo = res.data.rows
						this.soDetailList=res.data.rows.soDetailList
						console.log(res.data.rows.soDetailList)
						...
						this.receivedAmount=data.receivedAmount//应付金额
						this.payAmount=data.payAmount//已收金额
						this.discountAmount=data.discountAmount//优惠金额
						...
						this.logisticsNo=data.logisticsNo//物流编号
						...
						// this.salesInfoadd=res.data.rows//复制新增详情
				...
				//获取销售订单明细下方
				this.axios.get('/so/detail/'+id+'?uid='+this.uid).then((res)=>{
					if(res.data.status==200){
						...
						if(res.data.rows.length==0){
							...
							let totalP;//添加总价变量
							this.cgListInfo.forEach((item,index)=>{
			...
			examine(){
				this.$Modal.confirm({
                    ...
                    content: '<p>确定要审核吗?</p>',
                    onOk: () => {
                        this.axios.get('so/submit/'+this.id+'?uid='+this.uid).then((res)=>{
							if(res.status==200){
								this.$notify({
							        title: '',
							        message: '已审核',
			...
			cancel(){
				this.$Modal.confirm({
                    ...
                    content: '<p>确定要弃审吗?</p>',
                    onOk: () => {
                        this.axios.get('so/cancel/'+this.id+'?uid='+this.uid).then((res)=>{
							if(res.status==200){
								this.$notify({
							        title: '',
							        message: '已弃审',
			...
			invalid(id){
				this.$Modal.confirm({
                    ...
                    content: '<p>确定要作废吗?</p>',
                    onOk: () => {
                        this.axios.get('so/invalid/'+this.id+'?uid='+this.uid).then((res)=>{
							if(res.data.status==200){
									this.$notify({
								        title: '作废',
								        message:  '已作废',
			...
			deliveryClick(){
				this.fh=true
				this.addText=false
				this.$refs.fh.style.display="block"
				this.$refs.detail.style.display='none'
				this.editxiugai=false
				this.deliveryshow=false
				this.deliveryshowsave=true//发货保存按钮

				this.deliverProvince=this.deliverProvince
  				this.deliverCity=this.deliverCity
  				this.deliverCountry=this.deliverCountry
  				this.getallwarhose()
				//转为code
        		if(this.deliverProvince==null ||this.deliverProvince=='' || this.deliverCity==null|| this.deliverCity=='' || this.deliverCountry==null|| this.deliverCountry==''){
        			this.selectedOptions2=[]
			...
			//发货保存
			deliverysave(){
				if(this.cgListInfo[0].skuName=="选择商品"){
					console.log(this.cgListInfo)
					this.cgListInfo.splice(0,1)
					if(this.warehouseId==''){
						this.$notify({
					        title:"",//
					        message:"请选择发货仓库",
						...
						this.axios.post('ship/add/?uid='+this.uid,{
							soId:this.id,
		            		partnerName:this.xiaoshouformValidate.partnerName,//客户名称
							...
							shipmentDetailVos:this.cgListInfo,//发货明细
							recordDate :this.recordDate,//订单日期
							remark:this.remark,
							attachs:this.update,//多附件地址
					...
					if(this.warehouseId==''){
						...
						this.axios.post('ship/add/?uid='+this.uid,{
						soId:this.id,
							// salesType:this.cgxqInfo.salesType,//销售类型
		            		partnerName:this.xiaoshouformValidate.partnerName,//客户名称
			            	partnerId:this.partnerId,//客户id
			  				deliverName:this.xiaoshouformValidate.deliverName,
			  				deliverTel:this.xiaoshouformValidate.deliverTel,
			  				...
			  				deliverCity:this.deliverCity,
			  				deliverCountry:this.deliverCountry,
							shipmentDetailVos:this.addfharr,//发货明细
							recordDate :this.recordDate,//订单日期
							remark:this.remark,
							attachs:this.update,//多附件地址
							shipmentDetailVos:this.cgListInfo,//发货明细
			...
			//完成
			allsign(){
				this.axios.get('so/confirm/'+this.id+'?uid='+this.uid).then((res)=>{
					if(res.data.status==200){
						this.$notify({
					        title: '',
					        message: '完成',
				...
				this.$refs[name].validate((valid) => {
					if(valid){
							...
							if(this.soDetailList.length==0){
								...
								if((this.amountAll(this.soDetailList)-this.discountAmount-this.payAmount)<0){
						 			this.$notify({
								        title:"",//
								        message:'应收金额不能小于0',//
								        type: 'error',
						 			...
						 			this.axios.post('/so/add?uid=' + this.uid, {
										...
										fax:this.fax,
						  				...
						  				deliverCity:this.deliverCity,
						  				deliverCountry:this.deliverCountry,
										soDetailList:this.soDetailList,//采购明细
										recordDate :this.xiaoshouformValidate.recordDate ,//订单日期
										// recordNo:this.recordNo,
										receivedAmount:this.amountAll(this.soDetailList)-this.discountAmount-this.payAmount,//应收金额
										payAmount:this.payAmount,//已收金额
										discountAmount:this.discountAmount,//优惠金额
										totalAmount:this.amountAll(this.soDetailList),
			...
			//保存后订单
			getcgLists() {
				this.axios.get('/so/query?uid=' + this.uid+'&status=1').then((res) => {
					let data = res.data;
					if(res.data.status == '200') {
			...
			//获取全部销售订单
			getcgList(s) {
				this.axios.get('/so/query?uid=' + this.uid+'&status='+s).then((res) => {
					let data = res.data;
					if(res.data.status == '200') {
						this.Allpo = data.rows;
						this.pototal=data.total
			...
			//单击存货表格一行弹出商品弹出框，并获取商品数据GET /quotation/sku/{id} 关联商品
			clickchrow(data,index){
				if(this.xiaoshouformValidate.partnerName==''){
					...
					this.axios.get('/quotation/sku/'+this.partnerId+'?uid='+this.uid).then((res)=>{
						this.goods= res.data.rows
						this.pageSize = res.data.pageSize
						this.total = res.data.total
						this.goods.forEach((x,index)=>{
                            x.skuCode=this.goods[index].skuCode
							x.skuId=this.goods[index].skuId
							x.quotationId=this.goods[index].quotationId
							x.price=this.goods[index].price
							x.costPrice=this.goods[index].costPrice
							x.wholePrice=this.goods[index].wholePrice
							x.unit=this.goods[index].unit
							x.unitId=this.goods[index].unitId
							x.className=this.goods[index].className
							x.classId=this.goods[index].classId
							x.inventoryUp=this.goods[index].inventoryUp
							x.inventory=this.goods[index].inventory
							x.barcode=this.goods[index].barcode
							x.barnd=this.goods[index].barnd
							x.mainPhoto=this.goods[index].mainPhoto
							x.photos=this.goods[index].photos
							x.unitList=this.goods[index].unitList
							x.unitDesc=this.goods[index].unitDesc
							x.attrDetailList=this.goods[index].attrDetailList
							x.partnerId=this.goods[index].partnerId
							x.specs=this.goods[index].specs
			...
			makegoods(){
				...
				if(this.rowgoodsDataAll.length!=0){
					...
					let arr={
						...
						salesPrice:this.rowgoodsData.price,
			...
			xiugaiedit(){
				...
				this.axios.get('/so/get/'+this.id+'?uid='+this.uid).then((res)=>{
					...
					if(res.data.status==200){
						...
						//console.log(data)
							...
							this.xiaoshouformValidate.salesType=data.soType
							this.recordDate=data.recordDate //订单日期
							this.xiaoshouformValidate.recordNo=data.recordNo
							this.totalAmount=data.totalAmount
							this.totalCount=data.totalCount
                		...
                		if(this.deliverProvince==''|| this.deliverCity=='' || this.deliverCountry==''){
                			this.selectedOptions2=[]
                		}else{
                			this.selectedOptions2=[this.TextToCode[this.deliverProvince].code,this.TextToCode[this.deliverProvince][this.deliverCity].code,this.TextToCode[this.deliverProvince][this.deliverCity][this.deliverCountry].code]
                		}
				...
				//获取采购订单明细下方
				this.axios.get('/so/detail/'+this.id+'?uid='+this.uid).then((res)=>{
					...
					if(res.data.status==200){
						...
						if(res.data.rows.length==0){
							...
							this.soDetailList = res.data.rows
						}
					}
				})
			...
			saveEdit(name){
				this.$refs[name].validate((valid) => {
					if(valid){
						if(this.soDetailList[0].skuName=="选择商品"){
							this.soDetailList.splice(0,1)
							if(this.soDetailList.length==0){
								this.$notify({
							        title:"",//
							        message:"请添加销售商品",
								...
								if((this.amountAll(this.soDetailList)-this.discountAmount-this.payAmount)<0){
									...
									this.axios.put('so/update?uid='+this.uid,{
										id:this.id,
										salesType:this.xiaoshouformValidate.salesType,//销售类型
					            		partnerName:this.xiaoshouformValidate.partnerName,//供应商名称
						            	partnerId:this.partnerId,//供应商id
										...
										fax:this.fax,
						  				email:this.email,
						  				address:this.address,
						  				payType:this.xiaoshouformValidate.modeldztype,
						  				deliverName:this.xiaoshouformValidate.deliverName,
						  				deliverTel:this.xiaoshouformValidate.deliverTel,
						  				deliverMobile:this.xiaoshouformValidate.deliverMobile,
						  				deliverAddress:this.deliverAddress,
			...
			getlogistics(value){
				this.xiaoshouformValidate.model1 = value
			...
			searkh(){
			  ...
			  //商品模糊搜索
            searchInput(){
				this.axios.get('/quotation/sku/'+this.partnerId+'?keyword='+this.goodskeyword+'&uid='+this.uid).then((res)=>{
           ...
           enterSearch(){
            ...
            //获取日志记录
            getshjl(id){
	        	this.axios.get('so/log?id='+id+'&uid='+this.uid).then((res)=>{
						if(res.data.status==200){
            ...
            handleReachEdge(){
            	this.current++//默认10条
    	 		this.axios.get('/so/query?cl='+this.value+'&offset='+this.current+'&uid='+this.uid+'&status='+this.valueT).then((res)=>{
            ...
            //支付类型
            getpaytype(){
            	this.axios.get('paytype/query?length=10&uid='+this.uid).then(res=>{
            		if(res.data.status==200){
            ...
            changeSearch(q){
            	this.xiaoshouformValidate.salesType=q
            },
          ...
          //客户翻页
           ...
           //切换一条页
			...
			orderselected(value){
			 ...
			 //商品切换一条页
            getData(current){
                this.axios.get('/quotation/sku/'+this.partnerId+'?offset='+current+'&uid='+this.uid).then((res)=>{
              		if(res.data.status==200){
              			this.goods = res.data.rows
              			this.pageSize = res.data.pageSize
						this.total = res.data.total
            ...
            changePageSize(size){
             ...
             //明细切换一条页
            getData1(current){
                this.axios.get('/so/detail/'+this.id+'?offset='+current+'&uid='+this.uid).then((res)=>{
              		if(res.data.status==200){
              			this.cgListInfo = res.data.rows
						//console.log(res.data.rows)
						this.total1=res.data.total
           ...
           //点击取消回到详情
            ...
            delUp(item,index){
             ...
             //精准搜搜点击搜索
            searchdd(){
                this.axios.get('/so/query?keyword='+this.searchKeyword+'&uid=' + this.uid).then((res) => {
                    if(res.data.status==200){
                        let data = res.data;
                        this.Allpo = data.rows;
                        this.pototal=data.total
            ...
            shdatepoDate(date){
             ...
             //精准搜索确认
            sureSearchLocation(){
               this.axios.get('/so/query/?cl=0'+'&uid=' + this.uid,{
           ...
           //单击仓库列表一行
			...
			//仓库确认
			makeck(){
				this.warehouseName= this.ckObj.name
			...
			//获取所有仓库
			getallwarhose(){
				this.axios.get('transfer/ware?length=999&uid='+this.uid+'&status=1').then((res)=>{
			...
			//仓库模糊搜索
			getckKeyword(){
				this.axios.get('transfer/ware?length=999&keyword='+this.ckkeyword+'&uid='+this.uid+'&status=1').then((res)=>{
					if(res.data.status==200){
						this.allwarehouse = res.data.rows
              			this.cktotal=res.data.total
						this.ckpageSize=res.data.pageSize
			 ...
			 //仓库切换一条页
            getckData(current){
                this.axios.get('transfer/ware?length=999&offset='+current+'&uid='+this.uid+'&status=1').then((res)=>{
              		if(res.data.status==200){
              			this.allwarehouse = res.data.rows
              			this.cktotal=res.data.total
						this.ckpageSize=res.data.pageSize
		...
		//全选
		 ...
		 exportOrder(){
		  ...
		  //导入
            ...
            //发货清单GET /ship/query 发货单 this.id //soId
            getdeliveryListgetData(){
            	this.axios.get('/ship/query?soId='+this.id+'&uid='+this.uid).then(res=>{
            		if(res.data.status==200){
            			let data=res.data
            			this.deliveryListData=data.rows
            			this.deliveryListtotal=data.total
            			this.deliveryListpageSize=data.pageSize
            			this.deliveryListData.forEach(x=>{
            				x.Raddress=x.deliverProvince+x.deliverCity+x.deliverCountry+x.deliverAddress
            			})
            		}
            	})
            },
            //翻页
            deliveryListgetData(current){
            	this.axios.get('/ship/query?soId='+this.id+'&uid='+this.uid+'&offset='+current).then(res=>{
            		if(res.data.status==200){
            			let data=res.data
            			this.deliveryListData=data.rows
            			this.deliveryListtotal=data.total
            ...
            deliveryListchangePageSize(size){
            	this.deliveryListpageSize=size
            },
        },
        mounted() {
			...
			// $('.optionMain').append('<div style="width:100%;height:35px;line-height:35px;text-align:center;">添加</div>')
...
.textbg1{
   ...
   .head{
	...
	.bordercodeicon{
		...
		width:16px;
		height:16px;
		/*background:url(http://img.zsydian.com/icon/duihao1.png);*/
	}
	.borderimg{
		max-height:100%;
		height:100px;
