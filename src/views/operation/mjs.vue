								...
								<div class='yhq-content-right'>
									<div style=';margin:10px 20px;padding:20px;border:1px solid #EEEEF1;background:#FCFCFC'>
										<div class='yhq-content-right-list'>
											...
											<Input v-model='name' type="text" style="width:90%"></Input>
										</div>
										<!--优惠方式：-->
										<div style='margin:10px 0;padding-bottom:20px;border-bottom:1px solid #e4e4e4'>
											...
											<div style='display:flex;line-height:35px'>
												...
												<div style='margin-right:40px'>
													<Radio :label="$t('operation.yhqHome.DK')"></Radio>
										...
										<div style="display:flex;height:36px;line-height:36px;margin:10px 0">
											...
											<Select style="width:120px;margin-top:5px;margin-left:5px" size="small" placeholder="全部" @on-change="hyselected">
	 		...
	 		<Tabs  :animated="false" @on-click="changeTabs">
	 			<TabPane label="详细">
		        	<Card>
		                ...
		                <p><span>满减：</span><span>{{yhqObj.amount}} 元</span></p>
...
export default{
	...
	data(){
		return{
			...
			Allfl:[
					{
						label:this.$t('operation.mjs.mjslist'),//'满减送列表',
						value:0
					},
					// {
					// 	label:'优惠券核销',
			...
			coupon:[
				...
				{
					title:this.$t('operation.mjs.mjs'),//"满减送",
					ellipsis:true,
					align: 'center',
					width:150,
					key:"name"
	...
	methods:{
		...
		//获取所有满减送
		getAllcoupon(){
				this.axios.get('/mj/query?uid='+this.uid).then((res)=>{
					if(res.data.status==200){
			 ...
			 //切换一条页
            getData(current){
                this.axios.get('/mj/query?offset='+current+'&uid='+this.uid).then((res)=>{
           ...
           goodsModelchangePageSize(size){
            ...
            //保存
	        saveyhq(){
	        	if(this.fsl==this.$t('public.Nolimt')){//'发放数量不限制'
	        		this.allCount=0
            ...
            clickRow(data,value){
				...
				this.allSku=this.yhqObj.allSku
            ...
            start(){
				if(this.valId==''){
					...
					this.axios.post('mj/enabled/'+this.valId+'?uid='+this.uid).then(res=>{
						if(res.data.status==200){
            ...
            stop(){
				if(this.valId==''){
					...
					this.axios.post('mj/disabled/'+this.valId+'?uid='+this.uid).then(res=>{
						if(res.data.status==200){
							this.getAllcoupon()//获取所有优惠券
						}
					})
