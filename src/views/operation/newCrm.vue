				...
				<Tabs :animated="false"  style="background:#fff;width:100%;height:100%" @on-click="clickli">
			        <TabPane label="注册奖励">
					    <div>
							...
							<div class='yscontent'>
								<div class='list'>
									<span style='width:120px;text-align:right'>{{$t('member.marketingHome.sendM')}}：&nbsp;&nbsp;</span><!--赠送现金券：-->
								
									<InputNumber :min="0" v-model="chargeMoney" style='width:120px' @on-change="changechargeMoney"></InputNumber>
									&nbsp;&nbsp;{{$t('member.marketingHome.yuan')}}<!--元-->
								</div>
								<div class='list'>
									<span style='width:120px;text-align:right'>{{$t('member.marketingHome.xfDis')}}： &nbsp;&nbsp;</span><!--消费折扣-->
									
									<InputNumber :max="1" :min="0.1" :step="0.1"  v-model="discount" style='width:120px' @on-change="changechargediscount"></InputNumber>
									&nbsp;&nbsp;{{$t('member.marketingHome.Dis')}}<!--折-->
								</div>
								<div class='list'>
									<span style='width:120px;text-align:right'>{{$t('member.marketingHome.xfjf')}}：&nbsp;&nbsp;</span><!--消费积分-->
									
									<InputNumber :min="0"  v-model="mtintegral" style='width:120px' @on-change="changemtintegral"></InputNumber>
		...
		saveRegist(){
			this.axios.put('crmop/unew?uid='+this.uid,{
				...
				if(res.data.status==200){
					...
					this.chargeMoney=0//赠送现金券 
					this.discount=1//消费折扣 
					this.mtintegral=0//消费积分
					this.getRegist() 
				}else{
					this.$notify({
                        title:"",//
