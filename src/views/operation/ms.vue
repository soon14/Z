<template>
				    ...
				    <div>
						...
						<div class='addyhqmodel'>
							<div class='addyhqhead'>
								...
								<div>
									<Button v-if='isEdit1' shape="circle"  style="background:#3b77e3;margin-right:5px;color:#fff;border:none" @click="saveyhq" >
								...
								<div class='yhq-content-right'>
									<div style=';margin:10px 20px;padding:20px;border:1px solid #EEEEF1;background:#FCFCFC'>
										...
										<div style='margin:10px 0;padding-bottom:20px;border-bottom:1px solid #e4e4e4'>
											...
											<div style='display:flex;line-height:35px'>
												...
												<!-- <div style='margin-right:40px'>
													...
													<Input v-model='amount' number style="width:90px" placeholder='金额'></Input> 
													<span>元</span>
												</div> -->
												<div>
													<Radio  label="赠送"></Radio><!--消费满-->
													<span @click='getsku'>
													<Input v-model='skuname' number style="width:350px" :placeholder="$t('operation.yhqHome.choosesku')">
													</Input>
													</span>
												</div>
										...
										<div style="display:flex;height:36px;line-height:36px;margin:10px 0;border-top:1px solid #e4e4e4;margin:10px 0">
											<span style="width:100px">{{$t('operation.yhqHome.hdsku')}}：</span><!--活动商品-->
										...
										<!--活动店铺：-->
										<div style="display:flex;height:36px;line-height:36px;margin:10px 0">
											<span style="width:100px">{{$t('operation.yhqHome.hdstore')}}：</span>
	...
	<!--选择商品-->
	<Modal v-model="skuisshow" width="800px">
		...
		<div slot="footer" style='margin-top:-10px'>
			...
			<div class='title' style='margin:0px'>
				...
				<div>
					<Button shape="circle" type="ghost" style="display:inline-block;background:#ACACAC;border:none;color:#fff" @click='cancel'><Icon type="close" style="margin-right:5px;" ></Icon>{{$t('public.cancel')}}</Button>
	 		...
	 		<Tabs  :animated="false" @on-click="changeTabs">
	 			<TabPane label="详细">
		        	<Card>
		                ...
		                <p><span>赠送：</span><span>{{yhqObj.skuName}}</span></p>
...
export default{
	...
	data(){
		return{
			...
			Allfl:[
					{
						label:this.$t('operation.mjss.mjsslist'),//'优惠券列表',
						value:0
					},
					// {
					// 	label:'优惠券核销',
			...
			coupon:[
				...
				{
					title:this.$t('operation.mjss.mjss'),//"满送",
					ellipsis:true,
					width:150,
					align: 'center',
					key:"name"
	...
	methods:{
		...
		goaddgoods(){
			...
			this.allCount1=1//限制领取1
			this.allCount2=1////限制领取2
		...
		//获取所有满送
		getAllcoupon(){
				this.axios.get('/mjs/query?uid='+this.uid).then((res)=>{
					if(res.data.status==200){
						...
						this.couponList.forEach((item,index)=>{//skuId
							...
							this.axios.get('inventory/viewsku/'+item.skuId+'?uid='+this.uid).then(res=>{
								console.log(item)
								item.skuName=res.data.rows.skuName
							})
						})
					}
				})
        ...
        saveyhq(){
        	...
        	if(this.dk==this.$t('operation.yhqHome.man')){//x消费满
            ...
            clickRow(data,value){
            	...
            	this.name=this.yhqObj.name
				this.beginTime=this.yhqObj.beginTime
				this.endTime=this.yhqObj.endTime
				this.skuVoList=this.yhqObj.skuVoList
				this.skuIds=this.yhqObj.skuIds

				this.storeList=this.yhqObj.storeList
				this.storeIds=this.yhqObj.storeIds
				this.perCount=this.yhqObj.perCount
				this.levelIds=this.yhqObj.levelIds
				this.allStore=this.yhqObj.allStore

				this.skuid=this.yhqObj.skuId

				this.channelValue=this.yhqObj.channelLimit
				this.orderAmount=this.yhqObj.orderAmount
				this.amount=this.yhqObj.amount
				this.skuname=this.yhqObj.skuName


				this.useDateLimit=this.yhqObj.useDateLimit
				this.allCount=this.yhqObj.allCount
				this.description=this.yhqObj.description
				this.allCrm=this.yhqObj.allCrm
 				this.isEdit=true
            },
            //修改优惠券
            edityhq(){
           ...
           saveyhqE(){
            ...
            start(){
            	this.axios.post('mjs/enabled/'+this.valId+'?uid='+this.uid).then(res=>{
            		if(res.data.status==200){
            ...
            stop(){
            	this.axios.post('mjs/disabled/'+this.valId+'?uid='+this.uid).then(res=>{
            		if(res.data.status==200){
            			this.getAllcoupon()//获取所有优惠券
            		}
            	})
