<template>
	<div class="main">
		...
		<div class='order-left'>
			<div class='left-top'>
				<span style='font-size:14px;font-weight:700'>订单状态</span>
			</div>
			<div style="height:100%"  class='flclass'>
				<li class='flclass' v-for='(item,index) in orderType' @click='addBg(index,item)' :class="{bg:numIndex==index}" style='display:flex;justify-content: space-between'>
...
export default{
	...
	methods:{
		...
		addBg(index,item){
			...
			this.getAllOrder(this.valueT)
		},
		//筛选查询
		searchFilter(){
				this.axios.get('ordercross/query/0?uid='+this.uid,{
				params:{
		...
		//获取所有跨店订单
		getAllOrder(s){
				this.axios.get('ordercross/query/0'+'?uid='+this.uid+'&status='+s).then(res=>{
					console.log(res)
					if(res.data.status==200){
						this.AllOrderList=res.data.rows
						this.total=res.data.total
