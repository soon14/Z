<template>
		...
		<div class="head">
			...
			<!-- <Col span='18'> -->
				...
				<div style='margin-right:10px;position: absolute;right: 240px;'>
					<!-- <Button type="ghost"  shape="circle"  @click="orderpay" class='but' v-if='ispay'>
		...
		<div class="add-right" >
			...
			<div class='right-right' v-if='addTxt' style='position:relative'>
				<el-steps :active="active" finish-status="success" :align-center='true' style='margin-left:30px;margin-top:20px' v-if='activeIsShow'>
					...
					<el-step title="已支付" :description="createTime"></el-step>
				...
				<div class='top-info' >
					...
					<div style='margin-left:20px'>
						<li><span style='font-size:16px;color:#2B8F8F'>{{$t('orders.payTypeOrder')}}：</span >
						<span style='font-size:16px;color:blue'>{{payStatusDesc}}</span></li>
    ...
    <Modal v-model="wlisshow" width="420px">
        ...
        <FormItem label="物流费用" prop="amount"><!--物流费用-->
           ...
           <InputNumber  :min="1"  v-model="logisticsformValidate.amount" placeholder="物流费用"></InputNumber>
        </FormItem>
    	</Form>
	    <div slot="footer">
  ...
  <Modal v-model="iscancelorder" width="600px">
	    <p slot="header" style="height:30px;line-height:30px;text-align:center">
	      <span>{{$t('orders.cancelRemark')}}</span>
	    </p>
     	<div>
			 <Input type='textarea' v-model='iscancelRemark'></Input>
		 </div>
	    <div slot="footer">
	      <div class='footer-mark'>
	        <span><a style='color:#999;font-size:14px' @click='iscancelorderC'>{{$t('public.cancel')}}</a></span>
	        <span style='font-size:20px;width:1px;height:40px;background:#e4e4e4'></span>
	        <span ><a style='color:#3B77E3;font-size:14px' @click="sureaddressC">{{$t('public.sure')}}</a></span>
	      </div>
	    </div>
  </Modal>
	<div class='totBottom'>
...
import { regionData,CodeToText,TextToCode} from 'element-china-area-data'
	export default {
		...
		methods:{
			...
			//点击按钮状态改变后的列表
			getAllOrderS(){
					this.axios.get('ordercross/query/0?uid='+this.uid+'&status='+this.value).then(res=>{
			...
			orderSure(){
				if(this.orderStr==''){
					...
					this.axios.post('/ordercross/confrim?uid='+this.uid,this.orderStr).then(res=>{
						if(res.data.status==200){
							this.$notify({
								title:"",//
								message:this.$t('orders.orderYFH'),// '已确认',
								type: 'success',
								position: 'bottom-right'
							});
							this.slide()
	   ...
	   //          	})
			...
			//GET /ordercross/delivered/{id} 发货 出现物流弹框
			orderfh(){
				if(this.orderStr==''){
					this.$notify({
				        title:"",//
			...
			sureSearchpk(name){
				this.$refs[name].validate((valid) => {
					if(valid){
						this.axios.get('ordercross/delivered?uid='+this.uid,{
							params:{
								id:this.orderStr,
							...
							if(res.data.status==200){
								this.$notify({
									title:"",//
									message: this.$t('orders.orderDfh'),//'发货中',
									type: 'success',
									position: 'bottom-right'
								});
								this.slide()
			...
			orderYfh(){
				if(this.orderStr==''){
					...
					this.axios.post('/ordercross/sign?uid='+this.uid,this.orderStr).then(res=>{
						if(res.data.status==200){
							this.$notify({
								title:"",//
								message:"已收货",// '已发货',
			...
			//取消订单确认
			sureaddressC(){
				this.axios.post('/ordercross/cancel?uid='+this.uid,this.orderStr).then(res=>{
					if(res.data.status==200){
						this.$notify({
							title:"",//
			...
			//获取订单详情
			getCrmDetail(id){
				this.axios.get('ordercross/get/'+id+'?uid='+this.uid).then(res=>{
					if(res.data.status==200){
						...
						//取消
						if(rows.status==99 || rows.status==79 ){
							this.issure=false//确认
							this.isfh=false//发货中
							this.isyfh=false//已发货
							this.print=false//打印
			...
			orderselected(value){
				...
				this.value = value
				this.current=1
				this.getAllOrder(this.value)//全部线上订单
			},
			//翻页
			storegetData(current){
				this.axios.get('ordercross/query/0?offset='+current+'&uid='+this.uid+'&status='+this.value).then((res)=>{
					let data = res.data
			...
			storechangePageSize(size){
				this.axios.get('ordercross/query/0?length='+size+'&uid='+this.uid+'&status='+this.value).then((res)=>{
					let data = res.data
					this.AllOrderList = data.rows;
					this.total=res.data.total
					this.pageSize=res.data.pageSize
            ...
            handleReachEdge(){
				this.current++//默认10条
				//全部订单
				this.axios.get('ordercross/query/0?offset='+this.current+'&uid='+this.uid+'&status='+this.value).then((res)=>{
			...
			//获取订单日志
			getlogDate(){
				this.axios.get('/ordercross/orderlog/'+this.orderno+'?uid='+this.uid).then(res=>{
					if(res.data.status==200){
						this.logDate=res.data.rows.rows
						this.logDate.forEach(x=>{
			...
			//搜索订单
			search(){
				this.axios.get('ordercross/query/0?uid='+this.uid+'&keyword='+this.searchKeyword).then(res=>{
					if(res.data.status==200){
						this.AllOrderList=res.data.rows
						this.total=res.data.total
						this.pageSize=res.data.pageSize
