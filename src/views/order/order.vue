	...
	<div class="main">
		...
		<div style='display:flex;height: 100%;width:100%;overflow:hidden'>
			<div class='order-left'>
				<div class='left-top'>
					<span style='font-size:14px;font-weight:600'>{{$t('orders.orderStatus')}}</span><!--订单状态-->
				</div>
				<div style="height:100%"  class='flclass'>
					<li class='flclass' v-for='(item,index) in orderType' :key='index' @click='addBg(item,index)' :class="{bg:numIndex==index}" style='display:flex;justify-content: space-between'>
						...
						<div style='margin-right:10px' v-if="item.num!=0">{{item.num}}</div>
				...
				<div class='right-head'>
						...
						<div style='margin-top:10px;margin-left:20px'>
							<div style='display:flex;height:40px;line-height:40px'>
								...
								<DatePicker type="date"  :value='searchDate' style="width: 140px" placeholder="开始时间" @on-change='searchDate1'></DatePicker>
								<span style='margin-left: 6px;width: 80px;'>{{$t('operation.yhqHome.endTime')}}：</span>
								<DatePicker type="date"  :value='searchDate2' style="width: 140px;" placeholder="结束时间" @on-change='searchDate21'></DatePicker>
					...
					<div class='table-head1' v-for='(item,index) in AllOrderList' :key="index" v-else>
						<div class='table-sub-head'>
							<div class='Checkbox'>
								...
								<span style='margin-left:10px'>{{getLocalTimeH(item.createTime)}}</span>
							</div><!--选中-->
						...
						<div class='table-sub-order-list' v-for='(i,j) in item.details' :key="j">
							<li style='flex:1 0 300px;text-align:left;position:relative'>
								...
								<span class='goodsPrice'>
									<span style='color:#d53c39'>￥:{{i.price}} 元</span>
								</span>
							...
							<li>
								<span style='color:#d53c39'>￥：{{(i.qty*i.price).toFixed(2)}} 元</span>
							</li>
						</div>
...
export default{
	data(){
		...
		return {
			...
			orderType:[//0：全部 1：未支付 2：未确认 3：未发货 4：未收货 5：已完成 6：已取消 7：已关闭
					{
						label:"全部",
						value:0,
					},
	...
	methods:{
		...
		addBg(item,index){
			this.numIndex=index;
			this.valueT=item.value;
			this.orderstatus=item.value;
			if(this.valueT==0){
				this.getAllOrder(0);//线上订单
			}else{
				this.getAllOrderS(item.value);
			}
		...
		getAllOrder(){
			this.loadingisshow=true
			this.axios.get('orderon/query/'+this.valueT+'?uid='+this.uid).then(res=>{
				if(res.data.status==200){
					this.AllOrderList=res.data.rows
					this.total=res.data.total
					this.pageSize=res.data.pageSize
		...
		searchFilter(){
			this.loadingisshow=true
				this.axios.get('orderon/query/0?uid='+this.uid,{
				params:{
		...
		clickSeven(){
			this.loadingisshow=true
		  this.axios.get('/order/query/0'+'?uid='+this.uid,{
			  params:{
	 ...
	 //切换一条页this.orderstatus
        getData(current){
        	...
        	if(this.valueT==0){
        		...
        		this.axios.get('/orderon/query/0?offset='+current+'&uid='+this.uid+'&status='+this.valueT).then((res)=>{
				 	if(res.data.status==200){
				 		this.AllOrderList = res.data.rows
						this.total=res.data.total
						this.pageSize=res.data.pageSize
        ...
        changePageSize(size){
        	...
        	this.loadingisshow=true
			if(this.valueT==0){
        		this.axios.get('/orderon/query/0?length='+this.pageSize+'&uid='+this.uid).then((res)=>{
				 	if(res.data.status==200){
				 		this.AllOrderList = res.data.rows
						this.total=res.data.total
						this.pageSize=res.data.pageSize
        		...
        		this.axios.get('/orderon/query/0?length='+this.pageSize+'&uid='+this.uid+'&status='+this.valueT).then((res)=>{
				 	if(res.data.status==200){
				 		this.AllOrderList = res.data.rows
						this.total=res.data.total
						this.pageSize=res.data.pageSize
        ...
        //头部数字看板
	    getOrderOnIndex(){
	       	this.axios.get('/orderon/index?uid='+this.uid).then(res=>{
	       		if(res.data.status==200){
	       			let data=res.data.rows
	  				this.topSucess=data.finish//已完成
					this.topNosure=data.waitConfirm//待确认订单
					this.topNofh=data.noDelivery//未发货
					this.topNosh=data.noSign//未收货
					this.topNopay=data.waitPay//待支付订单
					this.topclose=data.colse//关闭订单
					this.topcancel=data.cancel//取消订单
					this.topAll=data.allOrder//全部
	       		}
	       	})
	    },
	...
	created(){
		this.$nextTick(()=>{
			this.getOrderOnIndex()//头部数字看板
			this.orderType[0].num=this.topAll
			this.orderType[1].num=this.topNopay
			this.orderType[2].num=this.topNosure
			this.orderType[3].num=this.topNofh
			this.orderType[4].num=this.topNosh
			this.orderType[5].num=this.topSucess
			this.orderType[6].num=this.topclose
			this.orderType[7].num=this.topcancel
			
		})
	},
	mounted(){
		this.getAllOrder()//所有线上订单
	}
}
</script>
