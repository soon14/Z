	...
	<div class="main">
		...
		<div style='display:flex;height: 100%;width:100%;overflow:hidden'>
			<div class='order-left'>
				<div class='left-top'>
					<span style='font-size:14px;font-weight:700'>{{$t('orders.orderStatus')}}</span><!--订单状态-->
				</div>
		...
		clickThirty(){
		 ...
		 //切换一条页
            getData(current){
            	this.loadingisshow=true
            	if(this.valueT==0){
            		this.axios.get('/order/query/0?offset='+current+'&uid='+this.uid).then((res)=>{
	                	if(res.data.status==200){
	                		this.AllOrderList = res.data.rows
		                    this.total=res.data.total
							this.pageSize=res.data.pageSize	
            		...
            		this.axios.get('/order/query/0?offset='+current+'&uid='+this.uid+'&status='+this.valueT).then((res)=>{
	                	if(res.data.status==200){
	                		this.AllOrderList = res.data.rows
		                    this.total=res.data.total
							this.pageSize=res.data.pageSize	
            ...
            changePageSize(size){
            	...
            	this.loadingisshow=true
            	if(this.valueT==0){
            		this.axios.get('/order/query/0?length='+this.pageSize+'&uid='+this.uid).then((res)=>{
	                	if(res.data.status==200){
	                		this.AllOrderList = res.data.rows
		                    this.total=res.data.total
							this.pageSize=res.data.pageSize
            		...
            		this.axios.get('/order/query/0?length='+this.pageSize+'&uid='+this.uid+'&status='+this.valueT).then((res)=>{
	                	if(res.data.status==200){
	                		this.AllOrderList = res.data.rows
		                    this.total=res.data.total
							this.pageSize=res.data.pageSize
