<template>
	<div>
		...
		<div class="left-content">
			...
			<p v-if="dataList.length==0"  class='scrollFix'>
				<Spin>
				...
				<div class="con-list" v-for="(item,index) in dataList" @click="dclick(item.skuId,index)" :class="{bg:numIndex==index}">
					<span>
					 ...
					 <div>
						...
						<span class="date">吊牌价 <span class="price-color">{{item.price}}</span></span>
						<span class="date1 ">库存 <span class="price-colorInv">{{item.qty}}</span></span>
					</div>
				</div>
...
export default{
	...
	methods:{
		search(){
			this.axios.get('inventory/query?keyword='+this.searchKeyword+'&uid='+this.uid).then(res=>{
