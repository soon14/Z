<template>
	<div>
	        ...
	        <div style='width:100%;background:#fff;overflow: hidden;'>
				...
				<div class="detail-bottom" v-show="addText">
					...
					<!--tab切换规格-->
						...
						<div v-if='num2==0'>
						    <div class="z-a-o">
						    	<span class="z-a-o-attr">属性</span>
						    	<span>选项</span>
						    </div>
						    <div class="z-a-o">
						    	<div class="z-a-o-attr">
									<Select v-model="attrsLeftName"  filterable :placeholder="placeholder" @on-change="getcgType" @on-query-change="changeSearch">
								        <Option v-for="(item,index) in moreAttrss" :value="item.value" :key="index">{{ item.label }}</Option>
								    </Select>
							    </div>
							    <div class="z-a-o-attr-r">
							    	<Input ></Input>
							    </div>
						    </div>
							<div style='color:#328FFE;margin:10px 0;cursor:pointer' @click="addmoreAttrss">
								<Icon type="plus" style='margin-right:5px'></Icon>
								<span >添加更多属性</span>
							</div>

							<!-- <div class='gg-border1'  v-for='(i,index) in addAttrs' >
								<span class='first'>
									{{i.name}}
								</span>
								<div style='width:100%;padding:10px 0;border-right:1px solid #ccc;border-left:1px solid #ccc'>
									
									<span class='two' v-for='(j,k) in i.detailVos' style='margin-left: 10px;'>
										<input type='checkbox' v-model="j.checked"  @change='changeAttr($event,i,j)' />{{j.name}}
									</span>
								</div>
								<span class='three' @click='alertEdit(i)'>{{$t('manage.sku.edit')}}</span>
							</div> -->
							<!--选择规格后的数据-->
							<div style='margin-top:20px;'>
								<Table  border highlight-row :columns="chooseGoodskey" :data="chooseGoodData">
								</Table>
							</div>
						</div>
						<div v-show='num2==1'>
			    ...
			    <div class="detail-bottom" v-show="xiugai">
					<div class="detail-public">
						...
						<div v-if='num2==0'>
							<div class='gg-border'>
								<span class='first'>{{$t('manage.sku.attr')}}</span><!--规格-->
								<span class='two'  v-for='(item,index) in attrsgoods1' @click='clickCheckbox(item,index,$event)' style='cursor:pointer;'>
									<span >
										<input type='checkbox' v-model='item.checked' @change='editselectedAttr($event,item,index)'/>
										{{item.name}}
									</span>

								</span>
								<span class='three' @click='addNewAttr'>{{$t('public.addnew')}}</span><!--新增规格-->
							</div>

							<div class='gg-border1'  v-for='(i,index) in attrsgoods1' v-if="i.checked"><!--attrsgoods1 addAttrs-->
								<span class='first'>
									{{i.name}}
								</span>
								<div style='width:100%;padding:10px 0;border-right:1px solid #ccc;border-left:1px solid #ccc'>
								<!--子规格-->
								<span class='two' v-for='(j,k) in i.detailVos' >
									<span >
										<input type='checkbox' v-model='j.checked' @change='editchangeAttr($event,i,j)' />{{j.name}}
									</span>
								</span>

								</div>
								<span class='three' @click='alertEdit(i)'>{{$t('manage.sku.edit')}}</span><!--编辑-->
							</div>
							<!--选择规格后的数据display:none-->
							<div style='margin-top:20px;'>
								<Table  border highlight-row :columns="chooseGoodskey" :data="chooseGoodData">
								</Table>
						...
						<!--详情-->
						<div v-show='num2==1' style='margin-right:10px'>
				...
				<div v-if='isdetails' style='height:98%;margin-left:7px;overflow:auto'>
		            ...
		            <!--规格-->
		            <div v-if='num4==0' style='margin-bottom:100px'>
						<div class='gg-border' v-if='specsArr==null || specsArr.length!=0'>
							<span class='first'>{{$t('manage.sku.attr')}}</span><!--规格-->
							<span class='two'  v-for='(item,index) in specsArr'  style='cursor:pointer;'>
								<span >
						...
						<!--无规格属性-->
						<div v-else class='noDataTxt'>
			                {{$t('public.NoData')}}
			            </div>
						<div class='gg-border1'  v-for='(i,index) in specsArr' v-if="i.checked" ><!--attrsgoods1 addAttrs-->
							<span class='first'>
								{{i.name}}
							</span>
							<!--子规格-->
							<div style='width:100%;padding:10px 0;border-right:1px solid #ccc;border-left:1px solid #ccc'>
								<span class='two' v-for='(j,k) in i.detailVos' >
									<span >
										<input type='checkbox' disabled v-model='j.checked' />{{j.name}}
									</span>
								</span>
							</div>
						</div>
						<!--选择规格后的数据-->
						<div style='width:99%;margin-top:20px;margin-bottom:170px'>
							<Table  border highlight-row :columns="chooseGoodskey1" :data="chooseGoodData">
							</Table>
						</div>
					</div>
					<!--详情-->
		            ...
		            <!--单位配置-->
					<div v-if='num4==2' style='margin-bottom:140px'>
						<div class='unitInfo'>
							<span> {{$t('manage.sku.about')}}：</span><!--说明-->
							<div>
								{{$t('manage.sku.about1')}}<!--只能设置一个库存单位,当库存单位设置为浮动换算时,只能设置一个辅单位-->。
					...
					<!--售后保障-->
					<div v-show='num4==4' style='margin-bottom:140px'>
		            ...
		            <!--自定义属性-->
			        <div  v-if='num4==6'  style='margin-bottom:140px'>
		...
		<!--编辑属性的弹框-->
		<Modal v-model="editAttrIshowSure" width="360px">
			<p slot="header" style="height:30px;line-height:30px">

				<span>{{$t('manage.sku.editAttr')}}</span><!--编辑属性-->
			</p>
			<div>
				<Form :label-width="80">
					<FormItem :label="$t('manage.sku.editAttrName')"><!--属性名称-->
	                    <Input v-model="editAttrName"  :placeholder="$t('manage.sku.editAttrName')"></Input>
	                </FormItem>
	                <FormItem :label="$t('manage.sku.childAttr')" v-for='(i,index) in editAttrSubArr' :key='index'>
	                    <Input v-model="i.name"  :placeholder="$t('manage.sku.childAttr')" @on-blur='changeSubAttr(i,$event)'></Input>
	                </FormItem>
				</Form>
			...
			<div slot="footer">
				...
				<div class='title' style='margin:0px'>
					...
					<div>
						<Button shape="circle" type="ghost" style="display:inline-block;background:#ACACAC;border:none;color:#fff" @click=''><Icon type="close" style="margin-right:5px;" ></Icon>{{$t('public.cancel')}}</Button>
						<Button shape="circle" type="ghost" style="display:inline-block;background:#00A7F5;color:#fff" @click="editAttrSure"><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>{{$t('public.sure')}}</Button>
					</div>
				</div>
			</div>
		</Modal>
		...
		<Modal v-model="attrsisshow" width="660px">
			<p slot="header" style="height:30px;line-height:30px">
				...
				<span>{{$t('public.addnew')}}</span><!--新建属性-->
			</p>
			<div style='margin-bottom:-20px;margin-top:-10px;display:flex;height:40px;line-height:35px'>
				<span style='width:40px'>{{$t('manage.sku.SkuName')}}：</span><Input v-model='attrName'></Input><!--名称-->
			</div>
			<div slot="footer">
				<!--属性表格-->
				...
				<div class='title' style='margin:0px'>
					...
					<div style='margin-top:10px'>
						...
						<Button shape="circle" type="ghost" style="display:inline-block;background:#3b77e3;color:#fff" @click="addattrssure" v-if='sx'><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>{{$t('public.sure')}}</Button>
						<Button shape="circle" type="ghost" style="display:inline-block;background:#40ca98;color:#fff" @click="editattrssure" v-if='!sx'><i class="el-icon-edit" style=";margin-right:5px;"></i>{{$t('public.EditBtn')}}</Button><!--修改-->
					</div>
				</div>
			</div>
		</Modal>
		...
		<Modal v-model="atisshow" width="660px">
			<p slot="header" style="height:30px;line-height:30px">
				...
				<span>{{$t('public.addnew')}}</span><!--新建属性规格-->
			</p>
			<div style=';display:flex;height:40px;line-height:35px'>
				<span style='width:40px'>{{$t('manage.sku.SkuName')}}：</span><Input v-model='attrName1'></Input><!--名称-->
			</div>
			<div style=';margin-top:10px;display:flex;height:40px;line-height:35px'>
				<span style='width:40px'>{{$t('manage.sku.attrcode')}}：</span><Input v-model='code'></Input>
			</div>
			<div style=';margin-top:10px;display:flex;height:40px;line-height:35px'>
				<span style='width:40px'>{{$t('manage.sku.attrsq')}}：</span><Input v-model='sequence' number></Input><!--排序-->
			</div>
			<div slot="footer" style='margin-top:-20px'>
	...
	export default {
		...
		data(){
			return {
				...
				titles1:[this.$t('manage.sku.titles1'),this.$t('manage.sku.titles2'),this.$t('manage.sku.titles4'),this.$t('manage.sku.titles5'),this.$t('manage.sku.titles6'),this.$t('manage.sku.titles7'),this.$t('manage.sku.titles3')],
				titles:[this.$t('manage.sku.titles1'),this.$t('manage.sku.titles2'),this.$t('manage.sku.titles4'),this.$t('manage.sku.titles5'),this.$t('manage.sku.titles6'),this.$t('manage.sku.titles3')],
				//编辑器提交的内容
				editor1:'editor1',
				editor2:'editor2',
		...
		methods:{
			...
			//选中商品属性
			selectedAttr(e,i,index){
				//console.log(i.detailVos)
				if(i.detailVos==null || i.detailVos.length==0){
					return
				}else{
					if(e.target.checked){
					...
					this.addAttrs.push(i)//选中一条属性加入到adAttrs
					this.attrsData.push(this.arr)//中间数组，存在选中后的数据
				...
				if(e.target.checked==false){
					...
					for(let j=0;j<this.attrsData.length;j++){
						if(this.attrsData[j].id==i.id){//判断id是否相同
							this.attrsData.splice(j,1)
							this.addAttrs.splice(j,1)
							...
							this.chooseGoodData=this.specCombinations();//页面显示的数据
						}
					}
				}
				}
			...
			changeAttr(e,item,j){
				...
				if(e.target.checked){
					...
					j.checked=true
					for(let i=0;i<this.attrsData.length;i++){
						//判断当前点击的是否是当前父级;循环判断当前点击的是否与循环出来的id相同
						if(this.attrsData[i].id==item.id){
							this.attrsData[i].detailVos.push(j)
							this.chooseGoodData=this.specCombinations();//页面显示的数据
							//////console.log(this.chooseGoodData)
				...
				if(e.target.checked==false){
					...
					//取消选中删除子属性
						...
						for(let n=0;n<this.attrsData.length;n++){
							for(let m=0;m<this.attrsData[n].detailVos.length;m++){
								if(this.attrsData[n].detailVos[m].id==j.id){

								this.attrsData[n].detailVos.splice(m,1)
								this.chooseGoodData=this.specCombinations();//页面显示的数据
								}
							}
			...
			//修改点击子属性选中
			editchangeAttr(e,item,j){
			////console.log(this.editChooseData)

				if(e.target.checked){

					j.checked=true
					for(let i=0;i<this.editChooseData.length;i++){
						//判断当前点击的是否是当前父级;循环判断当前点击的是否与循环出来的id相同
						if(this.editChooseData[i].id==item.id){
							this.editChooseData[i].detailVos.push(j)
							this.chooseGoodData=this.editspecCombinations();//页面显示的数据
				...
				if(e.target.checked==false){
					...
					//取消选中删除子属性
						...
						for(let n=0;n<this.editChooseData.length;n++){
							for(let m=0;m<this.editChooseData[n].detailVos.length;m++){
								if(this.editChooseData[n].detailVos[m].id==j.id){

								this.editChooseData[n].detailVos.splice(m,1)
								this.chooseGoodData=this.editspecCombinations();//页面显示的数据
								}
							}
		...
		specCombinations(){
				var arrWra=[]//组合后的数组
				let showText=[]//拼接组合后规格名称
			if(this.attrsData.length==1){//只有一条属性
				var	arr=this.attrsData[0].detailVos
					 if (arr.length == 0) {
			              arr = []
			            }
					 for (var e = 0; e < arr.length; e++) {
			              ...
			              _arr1={
			              	...
			              	wholePrice:Number(this.formValidate.wholePrice),
			              	costPrice:Number(this.formValidate.costPrice),
			              	specDetailName:arr[e].name,
			              	specDetailId:arr[e].id
			              }
			              arrWra.push(_arr1)
				     }
					return arrWra
				...
				//只有2条属性
			if(this.attrsData.length==2){
					var arr = this.attrsData[0].detailVos
	          		var arr1 = this.attrsData[1].detailVos
		            ...
		            for (var f = 0; f < arr.length; f++) {
		             	for (var g = 0; g < arr1.length; g++) {
		             		...
		             		_arr2={
			              	...
			              	wholePrice:Number(this.formValidate.wholePrice),
			              	costPrice:Number(this.formValidate.costPrice),
				              	specDetailName:arr[f].name+','+arr1[g].name,
				              	specDetailId:arr[f].id+','+arr1[g].id
				              }
				            arrWra.push(_arr2)
				...
				//只有3条属性
			if(this.attrsData.length == 3){
	          		var arr = this.attrsData[0].detailVos
	          		var arr1 = this.attrsData[1].detailVos
	          		var arr2 = this.attrsData[2].detailVos
		            if (arr.length == 0) {
		              arr = ['']
		            }
		            if (arr1.length == 0) {
		            ...
		            for (var f = 0; f < arr.length; f++) {
		             	for (var g = 0; g < arr1.length; g++) {
		             		for (var k = 0; k < arr2.length; k++) {
					            ...
					            _arr3={
					              	// price:this.Aprice,
			              			// wholePrice:this.AwholePrice,
			              			// costPrice:this.AcostPrice,
			              			// inventory:this.Aqty,
			              			price:Number(this.formValidate.price),
			              			wholePrice:Number(this.formValidate.wholePrice),
			              			costPrice:Number(this.formValidate.costPrice),
					              	specDetailName:arr[f].name+','+arr1[g].name+','+arr2[k].name,
					              	specDetailId:arr[f].id+','+arr1[g].id+','+arr2[k].id
					              }
					            arrWra.push(_arr3)
		            		}
		            	}
	          ...
	          //只有4条属性
	        if(this.attrsData.length == 4) {
	            var arr1 = this.attrsData[0].detailVos
	            var arr2 = this.attrsData[1].detailVos
	            var arr3 = this.attrsData[2].detailVos
	            var arr4 = this.attrsData[3].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	            if (arr2.length == 0) {
	               		...
	               		for (var d = 0; d < arr4.length; d++) {
	               			...
	               			_arr4={
			              		...
			              		price:Number(this.formValidate.price),
			              			wholePrice:Number(this.formValidate.wholePrice),
			              			costPrice:Number(this.formValidate.costPrice),
				              	specDetailName:arr1[a].name+','+arr2[b].name+','+arr3[c].name+','+arr4[d].name,
				              	specDetailId:arr1[a].id+','+arr2[b].id+','+arr3[c].id+','+arr4[d].id
				              }
			                arrWra.push(_arr4)
	              		}
	              	}
	           ...
	           return arrWra
				 ...
				 //只有5条属性
	        if(this.attrsData.length == 5) {
	            var arr1 = this.attrsData[0].detailVos
	            var arr2 = this.attrsData[1].detailVos
	            var arr3 = this.attrsData[2].detailVos
	            var arr4 = this.attrsData[3].detailVos
	            var arr5 = this.attrsData[4].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	               		...
	               		for (var d = 0; d < arr4.length; d++) {
	               			for(var e = 0;e<arr5.length;e++){
		               			...
		               			// 	price:this.Aprice,
			              			...
			              			wholePrice:Number(this.formValidate.wholePrice),
			              			costPrice:Number(this.formValidate.costPrice),
					              	specDetailName:arr1[a].name+','+arr2[b].name+','+arr3[c].name+','+arr4[d].name+','+arr5[e].name,
					              	specDetailId:arr1[a].id+','+arr2[b].id+','+arr3[c].id+','+arr4[d].id+','+arr5[e].id
					              }
				                arrWra.push(_arr5)
	               			}
	              		}
	           ...
	           //只有6条属性
	        if(this.attrsData.length == 6) {
	            var arr1 = this.attrsData[0].detailVos
	            var arr2 = this.attrsData[1].detailVos
	            var arr3 = this.attrsData[2].detailVos
	            var arr4 = this.attrsData[3].detailVos
	            var arr5 = this.attrsData[4].detailVos
	            var arr6 = this.attrsData[5].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	               		...
	               		for (var d = 0; d < arr4.length; d++) {
	               			for(var e = 0;e<arr5.length;e++){
	               				for(var f=0;f<arr6.length;f++){
			              			...
			              			wholePrice:Number(this.formValidate.wholePrice),
			              			costPrice:Number(this.formValidate.costPrice),
						              	specDetailName:arr1[a].name+','+arr2[b].name+','+arr3[c].name+','+arr4[d].name+','+arr5[e].name+','+arr6[f].name,
						              	specDetailId:arr1[a].id+','+arr2[b].id+','+arr3[c].id+','+arr4[d].id+','+arr5[e].id+','+arr6[f].id
						              }
					                arrWra.push(_arr6)
	               				}
	               			}
	        ...
	        if(this.attrsData.length == 7) {
	            ...
	            var arr5 = this.attrsData[4].detailVos
	            var arr6 = this.attrsData[5].detailVos
	            var arr7 = this.attrsData[6].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	               		...
	               		for (var d = 0; d < arr4.length; d++) {
	               			for(var e = 0;e<arr5.length;e++){
	               				for(var f=0;f<arr6.length;f++){
			              			...
			              			wholePrice:Number(this.formValidate.wholePrice),
			              			costPrice:Number(this.formValidate.costPrice),
							              	specDetailName:arr1[a].name+','+arr2[b].name+','+arr3[c].name+','+arr4[d].name+','+arr5[e].name+','+arr6[f].name+','+arr7[g].name,
							              	specDetailId:arr1[a].id+','+arr2[b].id+','+arr3[c].id+','+arr4[d].id+','+arr5[e].id+','+arr6[f].id+','+arr7[g].id
							              }
						                arrWra.push(_arr7)
	               					}
	               				}
	        ...
	        if(this.attrsData.length == 8) {
	            ...
	            var arr6 = this.attrsData[5].detailVos
	            var arr7 = this.attrsData[6].detailVos
	            var arr8 = this.attrsData[7].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	               		...
	               		for (var d = 0; d < arr4.length; d++) {
	               			for(var e = 0;e<arr5.length;e++){
	               				for(var f=0;f<arr6.length;f++){
			              			...
			              			wholePrice:Number(this.formValidate.wholePrice),
			              			costPrice:Number(this.formValidate.costPrice),
								              	specDetailName:arr1[a].name+','+arr2[b].name+','+arr3[c].name+','+arr4[d].name+','+arr5[e].name+','+arr6[f].name+','+arr7[g].name+','+arr8[h].name,
								              	specDetailId:arr1[a].id+','+arr2[b].id+','+arr3[c].id+','+arr4[d].id+','+arr5[e].id+','+arr6[f].id+','+arr7[g].id+','+arr8[h].id
								              }
							                arrWra.push(_arr8)
	               						}
	               					}
	        ...
	        if(this.attrsData.length == 9) {
	            ...
	            var arr7 = this.attrsData[6].detailVos
	            var arr8 = this.attrsData[7].detailVos
	            var arr9 = this.attrsData[8].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	               		...
	               		for (var d = 0; d < arr4.length; d++) {
	               			for(var e = 0;e<arr5.length;e++){
	               				for(var f=0;f<arr6.length;f++){
			              			...
			              			wholePrice:Number(this.formValidate.wholePrice),
			              			costPrice:Number(this.formValidate.costPrice),
									              	specDetailName:arr1[a].name+','+arr2[b].name+','+arr3[c].name+','+arr4[d].name+','+arr5[e].name+','+arr6[f].name+','+arr7[g].name+','+arr8[h].name+','+arr9[j].name,
									              	specDetailId:arr1[a].id+','+arr2[b].id+','+arr3[c].id+','+arr4[d].id+','+arr5[e].id+','+arr6[f].id+','+arr7[g].id+','+arr8[h].id+','+arr9[j].id
									              }
								                arrWra.push(_arr9)
	               							}
	               						}
	        ...
	        if(this.attrsData.length == 10) {
	            ...
	            var arr8 = this.attrsData[7].detailVos
	            var arr9 = this.attrsData[8].detailVos
	            var arr10 = this.attrsData[9].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	               							...
	               							for(var j=0;j<arr9.length;j++){
	               								for(var k=0;k<arr10.length;k++){
							               			...
							               			// 	price:this.Aprice,
			              								...
			              								wholePrice:Number(this.formValidate.wholePrice),
			              								costPrice:Number(this.formValidate.costPrice),
										              	specDetailName:arr1[a].name+','+arr2[b].name+','+arr3[c].name+','+arr4[d].name+','+arr5[e].name+','+arr6[f].name+','+arr7[g].name+','+arr8[h].name+','+arr9[j].name+','+arr10[k].name,
										              	specDetailId:arr1[a].id+','+arr2[b].id+','+arr3[c].id+','+arr4[d].id+','+arr5[e].id+','+arr6[f].id+','+arr7[g].id+','+arr8[h].id+','+arr9[j].id+','+arr10[k].id
										              }
									                arrWra.push(_arr10)
	               								}
	               							}
			...
			//修改的组合规格
		editspecCombinations(){
				var arrWra=[]//组合后的数组
				let showText=[]//拼接组合后规格名称
			if(this.editChooseData.length==1){//只有一条属性
				var	arr=this.editChooseData[0].detailVos
					 if (arr.length == 0) {
			              arr = []
			            }
					 for (var e = 0; e < arr.length; e++) {
				...
				//只有2条属性
			if(this.editChooseData.length==2){
					var arr = this.editChooseData[0].detailVos
	          		var arr1 = this.editChooseData[1].detailVos
				...
				//只有3条属性
			if(this.editChooseData.length == 3){
	          		var arr = this.editChooseData[0].detailVos
	          		var arr1 = this.editChooseData[1].detailVos
	          		var arr2 = this.editChooseData[2].detailVos
		            if (arr.length == 0) {
		              arr = ['']
		            }
		            if (arr1.length == 0) {
	          ...
	          //只有4条属性
	        if(this.editChooseData.length == 4) {
		            var arr1 = this.editChooseData[0].detailVos
		            var arr2 = this.editChooseData[1].detailVos
		            var arr3 = this.editChooseData[2].detailVos
		            var arr4 = this.editChooseData[3].detailVos
		            // 判断是否为[], 如果是 为其添加个空字符串占位
		            if (arr1.length == 0) {
		              arr1 = ['']
		            }
			...
			if(this.editChooseData.length == 5) {
	            var arr1 = this.editChooseData[0].detailVos
	            var arr2 = this.editChooseData[1].detailVos
	            var arr3 = this.editChooseData[2].detailVos
	            var arr4 = this.editChooseData[3].detailVos
	            var arr5 = this.editChooseData[4].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	          ...
	          //只有6条属性
	        if(this.editChooseData.length == 6) {
	            var arr1 = this.editChooseData[0].detailVos
	            var arr2 = this.editChooseData[1].detailVos
	            var arr3 = this.editChooseData[2].detailVos
	            var arr4 = this.editChooseData[3].detailVos
	            var arr5 = this.editChooseData[4].detailVos
	            var arr6 = this.editChooseData[5].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	          ...
	          //只有7条属性
	        if(this.editChooseData.length == 7) {
	            var arr1 = this.editChooseData[0].detailVos
	            var arr2 = this.editChooseData[1].detailVos
	            var arr3 = this.editChooseData[2].detailVos
	            var arr4 = this.editChooseData[3].detailVos
	            var arr5 = this.editChooseData[4].detailVos
	            var arr6 = this.editChooseData[5].detailVos
	            var arr7 = this.editChooseData[6].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	           ...
	           //只有8条属性
	        if(this.editChooseData.length == 8) {
	            var arr1 = this.editChooseData[0].detailVos
	            var arr2 = this.editChooseData[1].detailVos
	            var arr3 = this.editChooseData[2].detailVos
	            var arr4 = this.editChooseData[3].detailVos
	            ...
	            var arr6 = this.editChooseData[5].detailVos
	            var arr7 = this.editChooseData[6].detailVos
	            var arr8 = this.editChooseData[7].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	           ...
	           //只有9条属性
	        if(this.editChooseData.length == 9) {
	            var arr1 = this.editChooseData[0].detailVos
	            var arr2 = this.editChooseData[1].detailVos
	            var arr3 = this.editChooseData[2].detailVos
	            var arr4 = this.editChooseData[3].detailVos
	            ...
	            var arr7 = this.editChooseData[6].detailVos
	            var arr8 = this.editChooseData[7].detailVos
	            var arr9 = this.editChooseData[8].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
	          ...
	          //只有10条属性
	        if(this.editChooseData.length == 10) {
	            var arr1 = this.editChooseData[0].detailVos
	            var arr2 = this.editChooseData[1].detailVos
	            var arr3 = this.editChooseData[2].detailVos
	            var arr4 = this.editChooseData[3].detailVos
	            ...
	            var arr8 = this.editChooseData[7].detailVos
	            var arr9 = this.editChooseData[8].detailVos
	            var arr10 = this.editChooseData[9].detailVos
	            // 判断是否为[], 如果是 为其添加个空字符串占位
	            if (arr1.length == 0) {
	              arr1 = ['']
	            }
			...
			alertEdit(item){
				...
				this.attrsid=item.id
				this.detailVos1=item.detailVos

				// this.attrdata=item.detailVos
				if(item.detailVos==null || item.detailVos.length==''){
						this.attrdata=[{name:""}]
					}else{
						this.attrdata=item.detailVos
					}
			...
			//修改子属性输入框
			changeSubAttr(item,e){
				let arr={
					name:item.name,
					id:item.id
				}
				this.subeditDetailVos.push(arr)
				////////////console.log(this.subeditDetailVos)
			},
			//弹框的确认
			editAttrSure(){
				//每次确认之前清空subDetailVos

				this.axios.put('skuspec/update/?uid='+this.uid,{
					id:this.editAttrId,
					name:this.editAttrName,
					detailVos:this.subeditDetailVos
				}).then(res=>{
					if(res.data.status==200){
						this.editAttrIshowSure=false
						this.getmoreAttrs()//刷新属性
						this.subeditDetailVos=[]
					}else{
			...
			getGoodsInfo(id){
				this.axios.get('sku/get/'+id+'?uid='+this.uid).then((res)=>{
					if(res.data.status==200){
						...
						//获取返回的规格
							if(res.data.rows.specs==null || res.data.rows.specs.length==0){
								return
							}else {
								this.specsArr=res.data.rows.specs//商品规格//详情显示
								this.editChooseData=res.data.rows.specs//付给修改需要的组合数组
								this.chooseGoodData=this.editspecCombinations();//页面显示的数据
								////console.log(this.editChooseData)
								for(let j=0;j<this.specsArr.length;j++){

									for(let i=0;i<this.attrsgoods1.length;i++){
										if(this.attrsgoods1[i].id==this.specsArr[j].id)
										{
											this.attrsgoods1[i].checked=true
											this.specsArr[j].checked=true //详情显示
											let sonSpecsArr=this.specsArr[j].detailVos
										    let partnerDetailVos=this.attrsgoods1[i].detailVos
										    for(let m=0;m<sonSpecsArr.length;m++){
											    for(let n=0;n<partnerDetailVos.length;n++){
	                                              if(sonSpecsArr[m].id==partnerDetailVos[n].id)
	                                             	partnerDetailVos[n].checked=true
	                                             	sonSpecsArr[m].checked=true //详情显示
			...
			getmoreAttrs(){
				this.axios.get('skuspec/list?uid='+this.uid).then(res=>{
						if(res.data.status==200){
							this.moreAttrs=res.data.rows
							this.moreAttrs.forEach((x)=>{
								x.checked=false
								x.detailVos.forEach((i)=>{
									i.checked=false
								})
							})
						}
			...
			getmoreAttrs1(){
				this.axios.get('skuspec/list?uid='+this.uid).then(res=>{
					if(res.data.status==200){
						this.attrsgoods1=res.data.rows
		...
		addattrssure(){
			if(this.attrName==''){
				...
				this.$notify({
			        title: "",//'错误',
			        message:this.$t('public.valid.enterAtteName'),// '请输入属性名称',
			        type: 'error',
			        position: 'bottom-right'
		        });
			}else{
		...
		ggsure(){
			if(this.attrName1==''){
				this.$notify({
			        title: this.$t('public.success'),//'失败',
			        message: this.$t('public.valid.enterAtteName'),// '请输入属性名称',
			        type: 'error',
			        position: 'bottom-right'
		        });
		...
		ggcancel(){
	      ...
	      //精准搜索
            ...
            //添加更多属性
            addmoreAttrss(){
            	this.moreAttrss.push({
            		label
            	})
            },
            //选择属性下拉
            getcgType(value){
            	this.attrsLeftName = value
            },
            changeSearch(q){
            	this.attrsLeftName=q
            	console.log(q)
            },
