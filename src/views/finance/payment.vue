	...
	export default {
                ...
                importstore:[
                   {
					...
					{
						title: this.$t('finance.sk.ssMoney'),//'付款金额',
						ellipsis:true,
						key: 'amount'
					},
					{
		...
		methods: {
			...
			getUnitsList: function() {
			 ...
			 //点击搜索
            clickSearch(){
                this.axios.get('/payment/query?keyword='+this.keyword+'&uid=' + this.uid).then((res) => {
			...
			delUnits: function() {
				if(this.valId==''){
					...
					this.axios.delete('/payment/delete/' + this.valId + '?uid=' + this.uid).then((res) => {
			...
			// },
			 //切换一条页
            getData(current){
                this.axios.get('/payment/query?offset='+current+'&uid='+this.uid).then((res)=>{
                    //console.log(res)
