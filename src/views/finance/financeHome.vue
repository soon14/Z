	...
	<div>
		...
		<!--下方第一排图表-->
			...
			<div class='echarts ' style='margin-top:10px;position:relative;margin-bottom:120px'>
	        	...
	        	<div v-if='len41==0'  style="position:absolute;left:25%;;top:25%">{{$t('public.NoData')}}</div>
				<!--销售增长-->
		...
		drawxsbj (value){
			...
			if(value==1){
				this.axios.get('xsbb/mOrder?uid='+this.uid).then(res=>{
					if(res.data.status==200){
						this.len41=res.data.rows.morderChange.length
			...
			if(value==2){
				this.axios.get('/xsbb/order?uid='+this.uid).then(res=>{
					myEchartxsbj.showLoading()
					if(res.data.status==200){
						this.len41=res.data.rows.orderChange.length
			...
			if(value==3){
				this.axios.get('/xsbb/yOrder?uid='+this.uid).then(res=>{
					myEchartxsbj.showLoading()
					if(res.data.status==200){
						this.len41=res.data.rows.yorderChange.length
						let tm=[]//时间
						let qty=[]//数量
