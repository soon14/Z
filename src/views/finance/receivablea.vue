<template>
	...
	<div class="units-box">
		...
		<!--表格数据-->
		<div class="table">
			<Table border ref="selection" highlight-row :columns="columns" :data="dataList" @on-row-dblclick="dbl"  @on-row-click='clickRow'></Table>
		<!--底部的分页-->
		<div class="page-box">
...
import LoadUrl from '@/components/common/actionLoad' 
	export default {
      ...
      //点击搜索
            clickSearch(){
                this.axios.get('/receivable/query?keyword='+this.keyword+'&uid=' + this.uid).then((res) => {
			...
			delUnits: function() {
				if(this.valId==''){
					...
					this.axios.delete('/receivable/delete/' + this.valId + '?uid=' + this.uid).then((res) => {
			...
			// },
			 //切换一条页
            getData(current){
                this.axios.get('/receivable/query?offset='+current+'&uid='+this.uid).then((res)=>{
                    if(res.data.status == '200') {
                      this.isLoading=false
                      this.dataList = data.rows;
                      this.pageSize = data.pageSize;
