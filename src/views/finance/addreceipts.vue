		...
		<div class="cheng"></div>
					...
					<Scroll :on-reach-bottom="handleReachEdge" :height="height" v-else>
						<div class="con-list" v-for="(item,index) in Allunits" @click="updataUnit(item.id,index)" :class="{bg:numLeftList==index}">
							...
							<p class="txt" style="margin-top:10px;">
								<span style='display:inline-block;margin-top:10px'>{{item.partner.name}}</span>
							</p>
					...
					<div class="right-content"  v-show="addText">
						<div  style="width:100%;height:height:100%;padding:10px 0 10px 10px">
							 <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120">
							 	<Col span="11">
							 	 <FormItem :label="$t('finance.sk.supplier')" prop="khName"><!--供应商-->
							 	 	<span  @click='selekh' >
                                    	<Input v-model="formValidate.khName" :placeholder="$t('finance.sk.supplier')"></Input>
                                    </span>
                                </FormItem>
                                	<FormItem :label="$t('finance.sk.shzxm')" prop="szName"><!--收支项目-->
								 	 	<span  @click='selesz' >
	                                    	<Input v-model="formValidate.szName" :placeholder="$t('finance.sk.shzxm')"></Input>
	                                    </span>
                                	</FormItem>
                                <FormItem :label="$t('finance.skmoney')" prop="amount"><!--收款金额-->
                                ...
                                <Col span="11">
                                	<FormItem :label="$t('finance.sk.ey')" prop="ygName"><!--员工-->
								 	 	<span  @click='seleyg' >
	                                    	<Input v-model="formValidate.ygName" :placeholder="$t('finance.sk.ey')"></Input>
	                                    </span>
                                	</FormItem>
                                	<FormItem :label="$t('finance.payType')"><!--支付方式-->
			                        	<Select v-model="payType" @on-change="getdztype">
										<Option v-for="(item,index) in dztype" :value="item.key" :key="index">{{item.title}}</Option>
										</Select>
			                   		</FormItem>
                                	<FormItem :label="$t('finance.orderTime')"><!--单据日期-->
						...
						<div  style="width:100%;height:height:100%;padding:10px 0 10px 10px">
							<Form ref="formValidate1" :model="formValidate" :rules="ruleValidate" :label-width="120">
                                ...
                                <FormItem :label="$t('finance.skmoney')" prop="amount">
                                    	...
                                    	<Input v-model="formValidate.amount" :placeholder="$t('finance.skmoney')"></Input>
                                ...
                                <Col span="11">
                                	...
                                	<FormItem :label="$t('finance.payType')" >
			                        	<Select v-model="payType" @on-change="getdztype">
										<Option v-for="(item,index) in dztype" :value="item.key" :key="index">{{item.title}}</Option>
										</Select>
			                   		</FormItem>
			...
			<Modal v-model="khisshow" width="660px">
				...
				<div style="position:relative;text-align:center;width:100%;height:35px;line-height:35px;display:flex;">
					<Input v-model="gyskeyword" :placeholder="$t('public.codeandname')" style="padding:2px 0" @on-enter='enterSearch'>
					</Input>
	...
	export default {
		data() {
			return {
				...
				unitsText:this.$t('public.addnew'),//"新增收款单",//新增付款单
				addText:true,
		...
		methods: {
			...
			//获取收款单详情
			getUnitDefault(id){
					this.axios.get('/receipts/get/' + id + '?uid=' + this.uid).then((res) => {
	                    if(res.data.status == '200') {
			...
			saveBaseInfo(name) {
				this.$refs[name].validate((valid) => {
					if(valid){
						this.axios.post('/receipts/add?uid=' + this.uid, {
			...
			changeUnitInfo2(name){
				this.$refs[name].validate((valid) => {
					if(valid){
						this.axios.put('/receipts/update?uid='+this.uid,{
							id:this.id,
							amount:this.formValidate.amount,
		                	employeeId:this.employeeId,
		                	// partner:this.partner,
            ...
            delunit: function() {
               //修改的时候 删除  删除供应商
              //console.log(this.id)
               this.axios.delete('/receipts/delete/' + this.id + '?uid=' + this.uid).then(res=>{
               	if(res.data.status==200){
            ...
            handleReachEdge(){
				this.current++//默认10条
    	 		this.axios.get('/receipts/query?offset='+this.current+'&uid='+this.uid).then((res)=>{
