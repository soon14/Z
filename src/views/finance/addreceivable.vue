                 ...
                 <span style="position:absolute;left:380px;top:20px;font-weight:600;font-size:16px">
                    ...
                    <div class="right-top">
                        ...
                        <div>
                            ...
                            <Button type="primary" shape="circle" style="border:none;background:#3b77e3;color:#fff"  @click="saveBaseInfo('save')" v-show="addText"><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>{{$t('public.SaveBtn')}}</Button>
                             ...
                             <Button type="primary" shape="circle" style="background:#40ca98;margin-right:5px;color:#fff;border:none"  @click="changeUnitInfo1" v-show="xiugaibtn1"><i class="el-icon-edit" style=";margin-right:5px;"></i>{{$t('public.EditBtn')}}</Button>
                               <Button type="primary" shape="circle" style="background:#3b77e3;color:#fff;border:none"  @click="changeUnitInfo2('save1')" v-show="xiugaibtn2"><Icon type="edit" style="position:relative;right:5px"></Icon>{{$t('public.SaveBtn')}}</Button>
                           ...
                           <!--  <button @click="" v-show="addText" style="background:#D8D5DE;margin-right:20px;" >{{$t('public.cancel')}}</Button> -->
						</div>
		...
		<div class="cheng"></div>
				...
				<div class="left-content">
					...
					<Scroll :on-reach-bottom="handleReachEdge" :height="height" v-else>
						<div class="con-list" v-for="(item,index) in Allunits" @click="updataUnit(item.id,index)" :class="{bg:numLeftList==index}">
							...
							<span class="status" ><span style='color:red' >￥：{{item.payAmount}}元</span></span>
							...
							<Form ref="save" :model="formValidate" :rules="ruleValidate" :label-width="80" style='margin-left: 20px;'>
								...
								<Col span="11">
									<FormItem :label="$t('finance.sk.supplier')" prop="khName">
		                       		...
		                       		<FormItem :label="$t('finance.skmoney')" prop="amount">
								...
								<Col span="11">
									<FormItem :label="$t('finance.sk.ey')" prop="ygName">
										...
										<Input type="text" v-model="formValidate.ygName" ></Input>
										<Icon type="ios-search" size="16" style='position:absolute;top:8px;right:8px'></Icon>
									</div>
		                       		</FormItem>
		                       		<!-- <FormItem label="销售类型">
		                       		...
		                       		<FormItem :label="$t('finance.sk.payMoney')" prop="payAmount">
											<Input type="text" number v-model="formValidate.payAmount" ></Input>
		                       		</FormItem>
							...
							<Form ref="save1" :model="formValidate" :rules="ruleValidate" :label-width="80" style='margin-left: 20px;'>
								...
								<Col span="11">
									<FormItem :label="$t('finance.sk.supplier')" prop="khName">
		                       		...
		                       		<FormItem label="总金额" prop="amount">
											<Input type="text"  v-model="formValidate.amount" ></Input>
		                       		</FormItem>
		                       		<FormItem :label="$t('finance.skmoney')" prop="receivedAmount">
											<Input type="text"  v-model="formValidate.receivedAmount" ></Input>
		                       		</FormItem>
								...
								<Col span="11">
									<FormItem :label="$t('finance.sk.ey')" prop="ygName">
										<div  @click='seleyg' style='position:relative;'>
										<Input type="text" v-model="formValidate.ygName" ></Input>
										<Icon type="ios-search" size="16" style='position:absolute;top:8px;right:8px'></Icon>
									</div>
		                       		</FormItem>
			...
			<Modal v-model="khisshow" width="660px">
				...
				<div style="position:relative;text-align:center;width:100%;height:35px;line-height:35px;display:flex;">
					<Input v-model="gyskeyword"  style="padding:2px 0" @on-enter='enterSearchgys'>
				</Input>
	...
	export default {
		...
		methods: {
			...
			addUnit(){
				...
				this.formValidate.payAmount=0,//支付金额
			...
			//获取收款单详情
			getUnitDefault(id){
					this.axios.get('/payable/get/' + id + '?uid=' + this.uid).then((res) => {
	                    if(res.data.status == '200') {
	                    	...
	                    	this.formValidate.amount=String(data.amount)
			...
			saveBaseInfo(name) {
				this.$refs[name].validate((valid) => {
					if(valid){
						this.axios.post('/payable/add?uid=' + this.uid, {
			...
			changeUnitInfo2(name){
				this.$refs[name].validate((valid) => {
					if(valid){
						this.axios.put('/payable/update?uid='+this.uid,{
						id:this.id,
						amount:this.formValidate.amount,
	                	employeeId:this.employeeId,
	                	partnerName:this.formValidate.khName,
            ...
            delunit: function() {
               //修改的时候 删除  删除供应商
              //console.log(this.id)
               this.axios.delete('/payable/delete/' + this.id + '?uid=' + this.uid).then(res=>{
               	if(res.data.status==200){
            ...
            handleReachEdge(){
				this.current++//默认10条
    	 		this.axios.get('/payable/query?offset='+this.current+'&uid='+this.uid).then((res)=>{
	 				if(res.data.status==200){
