      ...
      <div class="heade"> 
                    ...
                    <div class="left-top">
                      <p class="wldw">
                            <!-- <span style="width:42px;font-weight:900">{{$t('public.screen')}}：</span>
                             <Select style="width:100px" size="small"  @on-change="orderselected">
                    ...
                    <div class="right-top">
                        ...
                        <div>
                            ...
                            <Button type="primary" shape="circle" style="margin-right:5px;border:none;background:#3b77e3;color:#fff"  @click="saveBaseInfo('save')" v-show="addText"><Icon type="checkmark-round" style=";margin-right:5px;color:#fff"></Icon>{{$t('public.SaveBtn')}}</Button>
                             ...
                             <Button type="primary" shape="circle" style="background:#40ca98;margin-right:5px;color:#fff;border:none"  @click="changeUnitInfo1" v-show="xiugaibtn1"><i class="el-icon-edit" style=";margin-right:5px;"></i>{{$t('public.EditBtn')}}</Button>
                               <Button type="primary" shape="circle" style="background:#3b77e3;margin-right:5px;color:#fff;border:none"  @click="changeUnitInfo2('save1')" v-show="xiugaibtn2"><Icon type="edit" style="position:relative;right:5px"></Icon>{{$t('public.SaveBtn')}}</Button>
		...
		<div class="cheng"></div>
				...
				<div class="left-content">
					...
					<Scroll :on-reach-bottom="handleReachEdge" :height="height" v-else>
						<div class="con-list" v-for="(item,index) in Allunits" @click="updataUnit(item.id,index)" :class="{bg:numLeftList==index}">
							...
							<!-- <span class="status" ><span style='color:red' >￥：{{item.amount}}元</span></span> -->
			...
			<!--右边保存与修改-->
				...
				<div class="add-right">
					<div class="right-content"  v-show="addText">
						<div  style="width:100%;height:height:100%;padding:10px 0 10px 10px">
							<Form ref="save" :model="formValidate" :rules="ruleValidate" :label-width="80" style='margin-left: 20px;'>
								<Row>
								<Col span="11">
									<FormItem :label="$t('finance.sk.supplier')" prop="khName">
		                            	<div @click='selekh' style='position:relative;'>
											<Input type="text" v-model="formValidate.khName" :placeholder="$t('finance.sk.supplier')"></Input>
		                       		...
		                       		<FormItem :label="$t('finance.sk.ysMoney')" prop="amount">
											<Input type="text"  v-model="formValidate.amount" ></Input>
		                       		</FormItem>
		                       		<FormItem :label="$t('finance.sk.ssMoney')" prop="receivedAmount">
											<Input type="text"  v-model="formValidate.receivedAmount"></Input>
		                       		</FormItem>
								...
								<Col span="11">
									<FormItem :label="$t('finance.sk.ey')" prop="ygName">
										<div  style="" @click='seleyg' style='position:relative;'>
										<Input type="text" v-model="formValidate.ygName" :placeholder="$t('finance.sk.ey')"></Input>
										<Icon type="ios-search" size="16" style='position:absolute;top:8px;right:8px'></Icon>
									</div>
		                       		</FormItem>
		                       		<!-- <FormItem label="销售类型">
											<Select placeholder="全部" @on-change="xiaoshouselected">
				                                <Option v-for="item in xiaoshouListname" :value="item.value" :key="item.value">{{ item.label }}</Option>
				                            </Select>
		                       		</FormItem> -->
		                       		<FormItem :label="$t('finance.sk.payMoney')" prop="payAmount">
											<Input type="text"  v-model="formValidate.payAmount" ></Input>
		                       		</FormItem>
		                       		<FormItem :label="$t('finance.sk.disc')" prop="discountAmount">
											<Input type="text"  v-model="formValidate.discountAmount" ></Input>
		                       		</FormItem>
		                       		<FormItem :label="$t('finance.sk.orderTime')">
											<DatePicker type="date" :value='recordDate' :placeholder="$t('finance.orderTime')"  style="width:100%" @on-change="shdate"></DatePicker>
		                       		</FormItem>
								</Col>
				...
				<div class="right-content"  v-show="xiugai">
						<div  style="width:100%;height:height:100%;padding:10px 0 10px 10px">
							<Form ref="save1" :model="formValidate" :rules="ruleValidate" :label-width="80" style='margin-left: 20px;'>
								<Row>
								<Col span="11">
									<FormItem :label="$t('finance.sk.supplier')" prop="khName">
		                            	<div @click='selekh' style='position:relative;'>
											
											<Input type="text" v-model="formValidate.khName" ></Input>
											<Icon type="ios-search" size="16" style='position:absolute;top:8px;right:8px'></Icon>
										</div>
		                       		</FormItem>
		                       		<FormItem :label="$t('finance.sk.shzxm')" prop="szName">
			...
			<Modal v-model="khisshow" width="660px">
				...
				<div style="position:relative;text-align:center;width:100%;height:35px;line-height:35px;display:flex;">
					<Input v-model="gyskeyword" @on-enter='enterSearch' :placeholder="$t('public.codeandname')" style="padding:2px 0">
					</Input>
			...
			<Modal v-model="ygisshow" width="660px">
				...
				<div style="position:relative;text-align:center;width:100%;height:35px;line-height:35px;display:flex;">
					<Input v-model="ygkeyword" placeholder="" style="padding:2px 0">
					</Input>
			...
			<Modal v-model="szisshow" width="660px">
				...
				<div style="position:relative;text-align:center;width:100%;height:35px;line-height:35px;display:flex;">
					<Input v-model="szkeyword" placeholder="员工名称/编码" style="padding:2px 0" @on-enter='enterSearchsz'>
					</Input>
	...
	export default {
		data() {
			return {
				...
				xiaoshouListname:[],//销售类型数组
				formValidate:{
					khName:'',//客户名称
					amount:0,//总金额
					receivedAmount:0,//应收金额
				...
				unitsText:this.$t('public.addnew'),//"新增收款单",//新增供应商
				addText:true,
		...
		methods: {
			...
			getUnitsList: function() {
				this.axios.get('/receivable/query?uid=' + this.uid).then((res) => {
					let data = res.data;
					//console.log(data)
			...
			addUnit(){
				...
				this.formValidate.receivedAmount=0,//应收金额
				this.formValidate.discountAmount=0,//折扣金额
				this.formValidate.payAmount=0,//折扣金额
			...
			//获取收款单详情
			getUnitDefault(id){
					this.axios.get('/receivable/get/' + id + '?uid=' + this.uid).then((res) => {
	                    if(res.data.status == '200') {
	                    	console.log(res.data.rows)
	                    	let data = res.data.rows;
	                    	this.storeDetails=data
	                    	this.formValidate.discountAmount=data.discountAmount
	                    	this.remark=data.remark
	                    	this.recordDate=data.recordDate.slice(0,data.recordDate.indexOf("."));
	                    	this.partnerId=data.partnerId  
	                    	this.finItemId=data.finItemId  
	                    	...
	                    	this.formValidate.payAmount=data.payAmount
	                    	this.formValidate.receivedAmount=data.receivedAmount
			...
			saveBaseInfo(name) {
				...
				this.$refs[name].validate((valid) => {
					if(valid){
						this.axios.post('/receivable/add?uid=' + this.uid, {
						amount:this.formValidate.amount,
	                	finItemId:this.finItemId,
	                	partnerName:this.formValidate.khName,
	                	partnerId:this.partnerId,
	                	recordDate:this.recordDate,
	                	remark:this.remark,
	                	payType:1,
	                	payAmount:this.formValidate.payAmount,
	                	receivedAmount:this.formValidate.receivedAmount,
			...
			changeUnitInfo2(name){
				this.$refs[name].validate((valid) => {
					if(valid){
						this.axios.put('/receivable/update?uid='+this.uid,{
						id:this.id,
						amount:this.formValidate.amount,
	                	finItemId:this.finItemId,
	                	partnerName:this.formValidate.khName,
            ...
            delunit: function() {
               //修改的时候 删除  删除供应商
              //console.log(this.id)
               this.axios.delete('/receivable/delete/' + this.id + '?uid=' + this.uid).then(res=>{
               	if(res.data.status==200){
            ...
            handleReachEdge(){
				this.current++//默认10条
    	 		this.axios.get('/receivable/query?offset='+this.current+'&uid='+this.uid).then((res)=>{
	 				if(res.data.statuw==200){
          ...
          // //设置日期
			...
			//员工模糊搜索
			getygKeyword(){
				this.axios.get('employee/query?keyword='+this.ygkeyword+'&uid='+this.uid).then((res)=>{
           	...
           	this.axios.get('partner/query?uid='+this.uid).then((res)=>{
           		if(res.data.status==200){
           			...
           			res.data.rows.forEach(item=>{
           				...
           				this.xiaoshouListname.push(arr)
           			})
           		}
           	})
           },
           ...
           //选择销售类型
           xiaoshouselected(v){
           	this.formValidate.recordType=v
           },
            //下拉框选择
            orderselected(value){
                // this.value = value
